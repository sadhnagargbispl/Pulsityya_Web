...
Partial Class App_UI_Application_Pages_Img
    ...
    " end as ImageLnk1 from M_Testimonials  where AId='" & Request("AId") & "'"
            ...
            ElseIf Request("Type") = "Payment" Then
                If Session("CompID") = 1091 Then
                    sql = "select top 1 case When ScannedFile='' then 'https://cpanel.solfit.in/Images/no_photo.jpg' else 'https://cpanel.solfit.in/images/UploadImage/'+ ScannedFile" & _
    " end as ImageLnk1 from WalletReq  where Reqno = '" & Request("ID") & "'"
                ElseIf Session("CompID") = 1074 Then
                    sql = "select case When ScannedFile='' then 'https://login.cashlessbazar.in/Images/no_photo.jpg' else 'https://login.cashlessbazar.in//images/UploadImage/'+ ScannedFile" & _
    " end as ImageLnk1 from WalletReq  where Reqno='" & Request("ID") & "'"
                ElseIf Session("CompID") = 1093 Then
                    sql = "Select case When ScannedFile='' then 'https://sollywood.in/images/no_photo.jpg' WHEN ScannedFile like 'http%' THEN "
                    sql &= " ScannedFile else 'https://sollywood.in/images/UploadImage/'+ ScannedFile end as ImageLnk1 from WalletReq  where Reqno = '" & Request("ID") & "'"
                ElseIf Session("CompID") = 1082 Then
                    sql = "select CASE WHEN ScannedFile='' THEN '' WHEN ScannedFile like 'http%' THEN ScannedFile else 'https://cpanel.cbullsecommerce.com/images/UploadImage/'+ScannedFile " & _
    " end as ImageLnk1 from WalletReq  where Reqno='" & Request("ID") & "'"
                ElseIf Session("CompID") = 1084 Then
                    sql = "select CASE WHEN ScannedFile='' THEN '' WHEN ScannedFile like 'http%' THEN ScannedFile else 'https://cpanel.ecoflowsglobal.com/images/UploadImage/'+ScannedFile " & _
    " end as ImageLnk1 from WalletReq  where Reqno='" & Request("ID") & "'"
                ElseIf Session("CompID") = 1089 Then
                    sql = "select CASE WHEN ScannedFile='' THEN '' WHEN ScannedFile like 'http%' THEN ScannedFile else 'https://megamart.ai/img/UploadImage/'+ScannedFile " & _
    " end as ImageLnk1 from WalletReq  where Reqno='" & Request("ID") & "'"
                ElseIf Session("CompID") = 1092 Then
                    sql = "select CASE WHEN ScannedFile='' THEN '' WHEN ScannedFile like 'http%' THEN ScannedFile else 'https://login.a7kart.com/images/UploadImage/'+ScannedFile " & _
    " end as ImageLnk1 from WalletReq  where Reqno='" & Request("ID") & "'"
                ElseIf Session("CompID") = 1102 Then
                    sql = "select CASE WHEN ScannedFile='' THEN '' WHEN ScannedFile like 'http%' THEN ScannedFile else 'https://cpanel.makeandgrowth.com/images/UploadImage/" & Session("CompId") & "/'+ScannedFile " & _
    " end as ImageLnk1 from WalletReq  where Reqno='" & Request("ID") & "'"
                ElseIf Session("CompID") = 1105 Then
                    sql = "select CASE WHEN ScannedFile='' THEN '' WHEN ScannedFile like 'http%' THEN ScannedFile else 'https://cpanel.powerlifewellness.com/images/UploadImage/" & Session("CompId") & "/'+ScannedFile " & _
    " end as ImageLnk1 from WalletReq  where Reqno='" & Request("ID") & "'"
                Else
    ...
    " end as ImageLnk1 from SKycVerify  where Formno='" & Request("ID") & "'"
        ...
        " end as ImageLnk1 from KycVerify  where Formno='" & Request("ID") & "'"
                ...
                ' ===================== COMPANY KYC (1108) - Light Bill (CompBillImg) =====================
            ElseIf Request("Type") = "CompBill" Then
                sql = "select case When Isnull(CompBillImg,'')='' then '" & url & "/Images/no_photo.jpg' else CompBillImg" & _
        " end as ImageLnk1 from KycVerify  where Formno='" & Request("ID") & "'"
                dt = New DataTable
                obj = New DAL(HttpContext.Current.Session("MlmDatabase" & Session("CompID")))
                dt = obj.GetData(sql)
                If dt.Rows.Count > 0 Then
                    Image1.ImageUrl = dt.Rows(0)("ImageLnk1")
                    HideAllKycDetailRows()
                End If

                ' ===================== COMPANY KYC (1108) - GST Certificate (CompGstImg) =====================
            ElseIf Request("Type") = "CompGst" Then
                sql = "select case When Isnull(CompGstImg,'')='' then '" & url & "/Images/no_photo.jpg' else CompGstImg" & _
        " end as ImageLnk1 from KycVerify  where Formno='" & Request("ID") & "'"
                dt = New DataTable
                obj = New DAL(HttpContext.Current.Session("MlmDatabase" & Session("CompID")))
                dt = obj.GetData(sql)
                ...
                ' ===================== COMPANY KYC (1108) - Other Proof (CompOtherImg) =====================
            ElseIf Request("Type") = "CompOther" Then
                sql = "select case When Isnull(CompOtherImg,'')='' then '" & url & "/Images/no_photo.jpg' else CompOtherImg" & _
        " end as ImageLnk1 from KycVerify  where Formno='" & Request("ID") & "'"
                dt = New DataTable
                obj = New DAL(HttpContext.Current.Session("MlmDatabase" & Session("CompID")))
                dt = obj.GetData(sql)
    ...
    ' Company KYC popup ke liye - saare member-detail rows hide, sirf image dikhe
    Private Sub HideAllKycDetailRows()
