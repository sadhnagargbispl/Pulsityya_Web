...
Partial Class MakeFranchise
    ...
    Private Sub FillCityPinDetail()
        Try
            ...
            sql &= " Where ActiveStatus = 'Y' And StateCode Not in (Select StateID from M_Makefranchise ) Order By StateNAme"
            objDAL = New DAL(HttpContext.Current.Session("MlmDatabase" & Session("CompID")))
    ...
    Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        Try
            ...
            Dim x As Integer = 0
            sql = " Insert Into M_Makefranchise (IDno,FormNo,StateID,StateName,Ractimestamp,ActiveStatus) "
            sql &= " Values('" & txtMemberId.Text.Trim() & "','" & hdnFormno.Value & "','" & ddlstate.SelectedValue & "',"
            sql &= " '" & ddlstate.SelectedItem.Text.Trim() & "',Getdate(),'Y');"
    ...
    Protected Sub txtMemberId_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtMemberId.TextChanged
        Try
            ...
            Else
                sql = " Select Formno,Prefix +' '+ MemFirstName As  MemNAme from M_MemberMaster Where IDNO = '" & txtMemberId.Text.Trim() & "' "
                objDAL = New DAL(HttpContext.Current.Session("MlmDatabase" & Session("CompID")))
                dt = objDAL.GetData(sql)
    ...
    Protected Sub ddlstate_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlstate.SelectedIndexChanged
        Try
            ...
            sql = " Select COUNT(*) cnt from M_Makefranchise Where StateID = '" & ddlstate.SelectedValue & "' "
            objDAL = New DAL(HttpContext.Current.Session("MlmDatabase" & Session("CompID")))
            dt = objDAL.GetData(sql)
            If (Val(dt.Rows(0)("cnt")) > 0) Then
                ddlstate.SelectedIndex = 0
                scrname = "<SCRIPT language='javascript'>alert('This state already exists in franchise.!!');" & "</SCRIPT>"
                ScriptManager.RegisterClientScriptBlock(Me.Page, Me.Page.GetType(), "alert", "alert('This state already exists in franchise.!!');", True)
            End If
        Catch ex As Exception
