...
public partial class Productrequestdetaildocket : System.Web.UI.Page
{
    string strquery;
    DataTable dt;
    ...
    private void PaymentDetails()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    string orderNo = dr["Orderno"].ToString().Trim();
                    string status = dt.Columns.Contains("Status") ? dr["Status"].ToString().Trim() : "";

                    // Bill type orderno (DV/26-27/WR/4) ya Dispatched => click pe bill download
                    if (status == "Dispatched" || orderNo.Contains("/"))
                    {
                        col2 = "<a href=\"https://franchise.dv9wellness.com/Invoice/DownloadPdf?Pm=" + Base64Encode(orderNo) +
                               "&id=" + Session["IDNo"] + "\" target=\"_blank\" style=\"color:blue\">" + orderNo + "</a>";
                    }
                    else
                    {
                        //    // Numeric pending orderno (1149754) => request detail
                        col2 = "<a href=\"#\" style=\"color:blue\">" + orderNo + "</a>";
                    }
