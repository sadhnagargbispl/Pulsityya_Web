...
Imports System.IO
Imports System.Net
Partial Class FundwithdrawalReportUpdate
...
"  B.RowStatus='Y' And B.ActiveStatus='Y' ,M_MemberMaster As C,m_sESSNmASTER AS D Where A.wsESSId=D.sESSID And A.FormNo=C.FormNo " & Condition & " Order by a.UserId, A.Formno"
    ...
    Protected Sub BtnExportToCsv_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnExportToCsv.Click
        ...
        Else
            Condition = Condition & " And a.Status='" & RbtStatus.SelectedValue & "'"
        End If
        sql = "SELECT ROW_NUMBER() OVER (ORDER BY a.ReqId DESC) AS [No.],a.PayeeName,CAST(''' ' + a.AcNo AS NVARCHAR(50)) AS AcNo,a.IFSCode AS [IFSCode],a.NetAmount AS Amount "
        sql &= " FROM Fundwithdrawls AS a INNER JOIN M_MemberMaster AS b ON a.Formno = b.Formno WHERE a.BankId = 63 AND a.Formno = b.Formno " & Condition & " ORDER BY a.ReqId DESC;"
        dtTemp = New DataTable
        dtTemp = objDAL.GetData(sql)
        ...
        If dtTemp IsNot Nothing AndAlso dtTemp.Rows.Count > 0 Then
            ...
            Dim totalRow As DataRow = dtTemp.NewRow()
            totalRow("AcNo") = "Total"        ' label "Total" (AcNo column ke neeche, screenshot jaisa)
            totalRow("Amount") = totalAmount  ' sum
            dtTemp.Rows.Add(totalRow)
        End If

        dg.DataSource = dtTemp
        dg.DataBind()
        ExportToExcel("HDFCExcelReport.xls", dg)
    End Sub
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Else
            Condition = Condition & " And a.Status='" & RbtStatus.SelectedValue & "'"
        End If
        sql = "SELECT CAST(ROW_NUMBER() OVER (ORDER BY a.ReqId DESC) AS NVARCHAR(20)) AS [No.],'''50200120794002' as [Remitter Account Number],'DV9 WELLNESS PRIVATE LIMITED' as [Remitter Account Name], "
        sql &= " a.IFSCode AS [IFSC code],a.NetAmount AS Amount,CAST(''' ' + a.AcNo AS NVARCHAR(50)) AS [Bank Account Number],a.PayeeName as [Beneficiary Name],'''00000000000000000000' as [Beneficiary LEI Code] "
        sql &= " FROM Fundwithdrawls AS a INNER JOIN M_MemberMaster AS b ON a.Formno = b.Formno WHERE a.BankId <> 63 AND a.Formno = b.Formno " & Condition & " ORDER BY a.ReqId DESC;"
        dtTemp = New DataTable
        dtTemp = objDAL.GetData(sql)
        ...
        If dtTemp IsNot Nothing AndAlso dtTemp.Rows.Count > 0 Then
            ...
            ' "TOTAL" label ko pehle NON-numeric (string) column mein daalo
            ...
            totalRow("Amount") = Math.Round(totalAmount, 2)
            dtTemp.Rows.Add(totalRow)
        End If
        dg.DataSource = dtTemp
        dg.DataBind()
        ExportToExcel("NonHDFCExcelReport.xls", dg)
    End Sub
