...
public partial class RepurchaseNow : System.Web.UI.Page
{
    string scrName;
    DAL ObjDAL;
    DAL obj;
    ...
    DAL Obj;
    public IEnumerable<G_RegisterUser> RegisterUserDetails { get; set; }
    ...
    protected void fillkit(string condition = "")
    {
        try
        {
            ...
            query = Obj.IsoStart + "Select KitId,KitName,KitAmount From " + Obj.dBName + "..M_KitMaster " +
                       "where ActiveStatus='Y' and Rowstatus='Y' and KitAmount <= (select Balance from dbo.ufnGetBalance('" +
                       Session["Formno"] + "','R')) and KitAmount > 0 " + condition + " Order By KitId" + Obj.IsoEnd;
            Dt_Kit = SqlHelper.ExecuteDataset(HttpContext.Current.Session["MlmSelectDatabase" + Session["CompID"]].ToString(), CommandType.Text, query).Tables[0];
    ...
    public string GenerateRandomStringactive(int iLength)
    {
        ...
        string numbers = "12345678"; // Sirf ye digits use hongi
    ...
    protected void IdActivation()
    {
        ...
        try
        {
            ...
            if (updateeffect > 0)
            {
                ...
                if (GetName() == "OK")
                {
                    ...
                    if (Convert.ToDecimal(Session["ServiceWallet"]) >= Convert.ToDecimal(txtAmount.Text))
                    {
                        ...
                        if (Dt_Kit.Rows.Count > 0)
                        {
                            string str1 = Obj.IsoStart + "Exec GetProducts 'WR','" + Dt_Kit.Rows[0]["ProductID"] + "'";
                            ...
                            if (Dt11.Rows.Count > 0)
                            {
                                ...
                                try
                                {
                                    ...
                                    if (updateeffect > 0)
                                    {
                                        ...
                                        {
                                            sql = "exec Sp_ActivateMemberRepurch '" + txtMemberId.Text.Trim() + "','" +
            ...
            {
                // Response.Redirect("RepurchaseNow.aspx");
                string scrName = "<SCRIPT>alert('Try Again.!');" + "location.replace('RepurchaseNow.aspx');</SCRIPT>";
    ...
    public IEnumerable<G_RegisterUser> SaveAddressDetail(string Action, string Id, string UserName, string Password, string Email, string FirstName, string Lastname, string Mobile, string FormNo,
          string StateCode, string District, string City, string Address, string PinCode, string AlternateMobileno, string BillingAddress, string BillingCity, string BillingPinCode, string BillingStateCodebState)
    {
        IEnumerable<G_RegisterUser> lst = null;
        try
        {
            BLLDBOperations blldb = new BLLDBOperations();
            Hashtable hst = new Hashtable();
            ...
            lst = DbOperation.ConvertDataTable<G_RegisterUser>(dt);
        ...
        try
        {
            ...
            if (Dt_.Rows.Count > 0)
            {
                if (Dt_.Rows[0]["ActiveStatus"].ToString() == "Y")
                {
                    ...
                    Session["St"] = Dt_.Rows[0]["ActiveStatus"].ToString();
                    return true;
                }
                else
                {
                    //string scrName = "<script>alert('Kindly activate your ID before continuing.');</script>";
                    //ScriptManager.RegisterClientScriptBlock(
                    //    this.Page, this.GetType(), "Upgraded", scrName, false);
                    //TxtMemberName.Text = "";
                    //LblMobile.Text = "";
                    string scrName =
                                "<SCRIPT>alert('Kindly activate your ID before continuing.!');" +
                                "location.replace('RepurchaseNow.aspx');</SCRIPT>";
                    ScriptManager.RegisterClientScriptBlock(
                               this.Page, this.GetType(), "Upgraded", scrName, false);
                    return false;
