...
Partial Class App_UI_Application_Pages_EpinIssuedValueReport
    ...
    Public Sub BindData(ByVal PageIndex As Integer)
        Try
            ...
            'Dim f As Integer = 0
            'Dim l As Integer
            '' sql = "select UserId,UserName from m_UserMaster where ActiveStatus='Y' and RowStatus='Y'"
            ...
            '    Session("IssuedPinValue") = dtData
            '    GvData.Visible = True
            '    gvContainer.Visible = True
            '    l = dtData.Rows.Count - 1
            '    If dtData.Rows.Count > 0 Then
            '        lblCount.Text = "Total Record: " & dtData.Rows.Count
            '        For f = 0 To l
            '            k = k + Val(dtData.Rows(f)("EpinValue"))
            '        Next
            '        LblTotalEpin.Text = "Total Epin Value :" & k
            '        LblTotalEpin.Visible = True
            '        btnExport.Enabled = True
            '        btnPrintAll.Enabled = True
            '        btnPrintCurrent.Enabled = True
            '    Else
            '        LblTotalEpin.Visible = False
    ...
    Private Sub ExportExcel()
        ...
        Using wb As New XLWorkbook()
            ...
            Response.AddHeader("content-disposition", "attachment;filename=EpinValueReport.xlsx")
    ...
    Private Sub FillUser()
        Try
            Dim sql As String = "Select UserName,UserID From M_Usermaster Where " + objDAL.activeCondition + " and IscustomerCare is Null  Order by UserName"
            dtData = New DataTable
            dtData = objDAL.GetData(sql)
            If dtData.Rows.Count > 0 Then
                CmbKit.DataSource = dtData
                CmbKit.DataTextField = "UserName"
                CmbKit.DataValueField = "UserID"
                CmbKit.DataBind()
            End If
        Catch ex As Exception
