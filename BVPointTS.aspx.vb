...
Partial Class BVPointTS
    ...
    Public Sub BindData()
        objDAL = New DAL(HttpContext.Current.Session("MlmDatabase" & Session("CompID")))
        Dim sql As String = "select  Cast(A.BID as varchar)as BID,a.Dsessid,Replace(Convert(Varchar,a.RectimeStamp,106),' ','-')as Date,b.Idno,(b.MemFirstName+' '+b.memLastName)as MemberName,Case when a.BVType='S' then 'Self' else 'Tree' end as BvType,  " & _
        " Case when a.LegNo=1 Then 'Left' else 'Right' end as LegNo, a.BV,a.Remark,Case when a.ActiveStatus='Y' then 'Active' else 'Deactive' " & _
        " end as Status ,a.PV from TrnBV as a,M_memberMaster as b where a.Formno=b.Formno Order By a.DSessid Desc,a.Formno Desc"
    ...
    Protected Sub GvData_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GvData.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If Val(Sessid) = Val(Format(Now, "yyyyMMdd")) Then
                GrpId = DirectCast(e.Row.Cells(9).FindControl("LBDelete"), LinkButton)
                If String.Equals(e.Row.Cells(8).Text.ToLower(), "deactive") = True Then
                    GrpId.Visible = False
                Else
                    GrpId.Visible = True
                End If
    ...
    Protected Sub DeleteGroup(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim str As String = "Select * from TrnBv where BId='" & Val(BankCode.ToString()) & "' and DSessid='" & Val(Format(Now, "yyyyMMdd")) & "' AND ActiveStatus='Y'"
        dt = New DataTable
        dt = objDAL.GetData(str)
        If dt.Rows.Count > 0 Then
            Dim Sql As String = "Update TrnBV SET ActiveStatus='N' WHERE BId='" & Val(BankCode.ToString()) & "'"
            If dt.Rows(0)("BvType") = "T" Then
                Sql = Sql & ";Insert into Repurchincome(Sessid,Formno,BillNo,Billdate,Repurchincome,Imported,BillType,SoldBy,MSessid,KitId,Dsessid,Remarks,PVValue)" & _
                            " values('" & Session("CurrentSessn") & "','" & Val(dt.Rows(0)("Formno")) & "','',Getdate(),'" & -Val(dt.Rows(0)("Bv")) & "','N','T','WR'," & _
                           "  '" & Session("CurrentSessn") & "' ,0,Convert(varchar,Getdate(),112),'Virtual BV Delete ','" & -Val(dt.Rows(0)("Pv")) & "') "
