        ...
        Try
            ...
            If RbtSearch.SelectedValue = "D" Then
                If txtStartDate.Text <> "" And txtEndDate.Text <> "" Then
                    ...
                    Condition2 = "and Cast(Replace(CONVERT(Varchar,b.Billdate,106),' ','-')as date)>=Cast('" & txtStartDate.Text & "' as date)  and Cast(Replace(CONVERT(varchar,b.BillDate,106),' ','-') as Date)<=Cast(Replace(Convert(Varchar,'" & txtEndDate.Text & "',106),' ','-') as Date)"
                    If DDlDate.SelectedValue = "J" Then
                        condition4 = "and Cast(Replace(CONVERT(Varchar,b.DOJ,106),' ','-')as date)>=Cast('" & txtStartDate.Text & "' as date)  and Cast(Replace(CONVERT(varchar,b.DOJ,106),' ','-') as Date)<=Cast(Replace(Convert(Varchar,'" & txtEndDate.Text & "',106),' ','-') as Date)"

                    Else
                        condition4 = "and b.activeStatus='Y' and Cast(Replace(CONVERT(Varchar,b.UpgradeDate,106),' ','-')as date)>=Cast('" & txtStartDate.Text & "' as date)  and Cast(Replace(CONVERT(varchar,b.UpgradeDate,106),' ','-') as Date)<=Cast(Replace(Convert(Varchar,'" & txtEndDate.Text & "',106),' ','-') as Date)"
                ...
                End If
            Else
                Condition2 = Condition2 & "and Cast(Replace(CONVERT(Varchar,b.BillDate,106),' ','-') as Date)>='" & LblSDate.Text & "' and cast(Replace(CONVERT(varchar,b.BillDate,106),' ','-') as Date)<='" & LblTDate.Text & "'"
            ...
            End If
            strquery = " select IsNull(Sum(temp.LeftBV),0) as LeftBV,Isnull(Sum(Temp.RightBV),0)as RightBV ," & _
            " IsnUll(sum(Temp.LeftJoining),0) as LeftJoining," & _
            " IsNull(sum(Temp.RightJoining),0) as RightJoining,IsNull(Sum(temp.LeftPV),0) as LeftPV,Isnull(Sum(Temp.RightPV),0)as RightPV From(" & _
            " SELECT  0 as LeftJoining,0 as RightJoining,Sum(c.repurchincome) AS LeftBV,0 as RightBv,0 as LeftPV,0 as RightPV    FROM   dbo.r_memtreerelation AS a" & _
            " INNER JOIN dbo.m_membermaster AS b ON a.formnodwn = b.formno  INNER JOIN repurchincome AS c " & _
            " ON c.formno = b.formno WHERE  a.legno % 2 = 1 AND c.billtype = 'R' and a.Formno='" & Val(FormNo) & "'" & Condition2 & " " & _
            " GROUP  BY a.formno  Union ALL" & _
             " SELECT  0 as LeftJoining,0 as RightJoining,0 AS LeftBV,Sum(c.repurchincome) as RightBv,0 as LeftPV,0 as RightPV    FROM   dbo.r_memtreerelation AS a" & _
            " INNER JOIN dbo.m_membermaster AS b ON a.formnodwn = b.formno  INNER JOIN repurchincome AS c " & _
            " ON c.formno = b.formno WHERE  a.legno % 2 = 0 AND c.billtype = 'R' and a.Formno='" & Val(FormNo) & "'" & Condition2 & " " & _
            " GROUP  BY a.formno  Union ALL" & _
              " SELECT Count(a.formnodwn) as LeftJoining,0 as RightJoining, 0 AS LeftBV,0 as RightBV ,0 as LeftPV,0 as RightPV FROM   " & _
              " dbo.r_memtreerelation AS a INNER JOIN dbo.m_membermaster AS b ON a.formnodwn = b.formno " & _
            "       WHERE  a.legno % 2 = 1  and a.Formno='" & Val(FormNo) & "' " & condition4 & " GROUP  BY a.formno " & _
            "  Union all SELECT 0 as LeftJoining,Count(a.formnodwn) as RightJoining, 0 AS LeftBV,0 as RightBV ,0 as LeftPV,0 as RightPV FROM   " & _
              " dbo.r_memtreerelation AS a INNER JOIN dbo.m_membermaster AS b ON a.formnodwn = b.formno " & _
            "       WHERE  a.legno % 2 = 0 and a.Formno='" & Val(FormNo) & "' " & condition4 & "  GROUP  BY a.formno " & _
  " Union All SELECT  0 as LeftJoining,0 as RightJoining,0 AS LeftBV,0 as RightBv ,Sum(c.pvVALUE) AS lEFTpv ,0 RightPV  FROM   dbo.r_memtreerelation AS a" & _
            " INNER JOIN dbo.m_membermaster AS b ON a.formnodwn = b.formno  INNER JOIN repurchincome AS c " & _
            " ON c.formno = b.formno WHERE  a.legno % 2 = 1 AND c.billtype <> 'R' and a.Formno='" & Val(FormNo) & "'" & Condition2 & " " & _
            " GROUP  BY a.formno  Union ALL" & _
             " SELECT  0 as LeftJoining,0 as RightJoining,0 AS LeftBV,0 as RightBv  ,0 as LeftPV,Sum(c.PVValue) as RightPV  FROM   dbo.r_memtreerelation AS a" & _
            " INNER JOIN dbo.m_membermaster AS b ON a.formnodwn = b.formno  INNER JOIN repurchincome AS c " & _
            " ON c.formno = b.formno WHERE  a.legno % 2 = 0 AND c.billtype <> 'R' and a.Formno='" & Val(FormNo) & "'" & Condition2 & " " & _
            " GROUP  BY a.formno  " & _
") as Temp"
            ...
            If Dt.Rows.Count > 0 Then
                ...
                LblCarryA.InnerText = Dt.Rows(0)("LeftJoining")
                LblCarryB.InnerText = Dt.Rows(0)("RightJoining")
                LblLeftPV.InnerText = Dt.Rows(0)("LeftPV")
                LBLRightPV.InnerText = Dt.Rows(0)("RightPV")
            End If
            RadioButton()
    ...
    Public Sub FillKit()
        Dim query As String = ""
        query = "Select kitId,KitName From M_KitMaster where RowStatus='Y'  Order By kitId"
    ...
    Private Sub FillDownline(Optional ByVal Condition As String = "", Optional ByVal IsSideA As Boolean = False)
        ...
        Try
            ...
            If RbtSearch.SelectedValue = "D" Then
                If txtStartDate.Text <> "" And txtEndDate.Text <> "" Then
                    If DDlDate.SelectedValue = "J" Then
                        ...
                        Condition2 = "and Cast(Replace(CONVERT(Varchar,Upgradedate1,106),' ','-')as date)>=Cast('" & txtStartDate.Text & "' as Date) and Cast(Replace(CONVERT(varchar,UpgradeDate1,106),' ','-') as Date)<=Cast('" & txtEndDate.Text & "' as date)"
                    Else
                        Condition2 = "and Cast(Replace(CONVERT(Varchar,UpgradeDate1,106),' ','-') as Date)>=Cast('" & txtStartDate.Text & "' as Date) and cast(Replace(CONVERT(varchar,UpgradeDate1,106),' ','-') as Date)<=Cast('" & txtEndDate.Text & "' as date) And ActiveStatus='Y'"
                    End If


                End If
            Else
                Condition2 = Condition2 & "and Cast(Replace(CONVERT(Varchar,UpgradeDate1,106),' ','-') as Date)>='" & LblSDate.Text & "' and cast(Replace(CONVERT(varchar,UpgradeDate1,106),' ','-') as Date)<='" & LblTDate.Text & "'"
            End If
            If ChkKit.Checked Then
                Condition2 = Condition2 & " And KitId='" & CmbKit.SelectedValue & "' "
            End If
            ...
            Dim Str As String = " Select Sum(lActive) as LeftActive,Sum(lDeactive) as LeftDeactive," & _
       ...
       " (select Count(Formno) as lActive,0 as lDeactive,0 as Ractive,0 as RDeactive from V#Downline" & _
         " where formno=" & Val(formNo.ToString()) & "  and legno % 2=1 and ActiveStatus='Y' " & Condition2 & "" & _
           " Union All" & _
        " select 0 as lActive,Count(Formno) as lDeactive,0 as Ractive,0 as RDeactive from V#Downline " & _
         " where formno=" & Val(formNo.ToString()) & "   and legno % 2=1 and ActiveStatus='N' " & Condition2 & "" & _
          " Union All" & _
        " select 0 as lActive,0 as lDeactive,Count(Formno) as Ractive,0 as RDeactive from V#Downline" & _
       " where formno=" & Val(formNo.ToString()) & "  and legno % 2=0 and ActiveStatus='Y' " & Condition2 & "" & _
            " union All" & _
         " select 0 as lActive,0 as lDeactive,0 as Ractive,Count(Formno) as RDeactive from V#Downline " & _
" where formno=" & Val(formNo.ToString()) & "  and legno % 2=0 and ActiveStatus='N' " & Condition2 & ") as Temp"
            dt = New DataTable
            dt = objDAL.GetData(Str)
            If dt.Rows.Count > 0 Then
                Leftactive.Text = "ODD Active: " & dt.Rows(0)("LeftActive")
                LeftDeactive.Text = "ODD Deactive: " & dt.Rows(0)("LeftDeactive")
                RActive.Text = "Even Active: " & dt.Rows(0)("RightActive")
                RDeactive.Text = "Even Deactive: " & dt.Rows(0)("RightDeactive")
            End If
            ...
            If rbleg.SelectedValue = "0" Or rbleg.SelectedValue = "1" Then
                ...
                strquery = "select top(100)*,doj +' '+JoiningTime  as DateofJoining,TopupDate+ ' '+TopUpTime as TopUp  from V#Downline where formno=" & Val(formNo.ToString()) & " and legno % 2=1 " & Condition2 & "  Order by JoinDate"
            ...
            If rbleg.SelectedValue = "0" Or rbleg.SelectedValue = "2" Then
                strquery = "select Top(100)*,doj +' '+JoiningTime  as DateofJoining,TopupDate+ ' '+TopUpTime as TopUp  from V#Downline where formno=" & Val(formNo.ToString()) & " and legno % 2=0 " & Condition2 & " order by JoinDate"
    ...
    Protected Sub BtnExportB_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnExportB.Click
        ...
        Dim str As String = "select  IDNO,MemName as MemberName,RefFormno as SponsorId,ReferalName as SponsorName,doj as Doj,JoiningTime,TopUpDate ,TopUpTime, KitName as Package,KitAmount as PackageAmount,Bv as Unit  from V#Downline where formno=" & Val(formNo.ToString()) & " and legno % 2=0 " & cond & " order by JoinDate"
    ...
    Protected Sub BtnExportA_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnExportA.Click
        ...
        End If
        ''Sponsor as UplinerId,SponsorName as UplinerName,
        Dim str As String = "select  IDNO,MemName as MemberName,RefFormno as SponsorId,ReferalName as SponsorName,doj as Doj,JoiningTime,TopUpDate ,TopUpTime, KitName as Package,KitAmount as PackageAmount,Bv as Unit    from V#Downline where formno=" & Val(formNo.ToString()) & " and legno % 2=1 " & cond & " order by JoinDate"
