...
Partial Class App_UI_Application_Pages_BankMaster
    ...
    Public Sub BindData()
        Try
            ...
            sql = "Select BankCode,Cast(BankCode as varchar) as VBankCode,BankName," & _
            " IFSCode," & _
            " AcNo,remarks,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status, Case when ActiveStatus='N' then 'label label-danger' else 'label label-success' end as StatusClass  From " + objDAL.tblBankMaster + " Where " + objDAL.activeCondition + " and bankcode>0 Order by BankName"
            ...
            If String.Equals(ddlSearchFields.SelectedItem.Text.ToLower(), "status") = True Then
                If String.IsNullOrEmpty(txtSearch.Text) = False Then
                    ...
                    Else
                        status = "Y"
                    End If
                    sql = "Select BankCode,Cast(BankCode as varchar) as VBankCode,BankName,IFSCode,AcNo,remarks,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status, Case when ActiveStatus='N' then 'label label-danger' else 'label label-success' end as StatusClass From " + objDAL.tblBankMaster + " Where " + objDAL.activeCondition + " AND ActiveStatus like '%" + status.ToString() + "%' Order by BankName"
                End If
            Else
                sql = "Select BankCode,Cast(BankCode as varchar) as VBankCode,BankName,IFSCode,AcNo,remarks,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status, Case when ActiveStatus='N' then 'label label-danger' else 'label label-success' end as StatusClass From " + objDAL.tblBankMaster + " Where " + objDAL.activeCondition + " AND " + ddlSearchFields.SelectedValue.ToString() + " like '%" + txtSearch.Text + "%' Order by BankName"
            End If
            dtData = New DataTable
            dtData = objDAL.GetData(sql)
            GvData.DataSource = dtData
