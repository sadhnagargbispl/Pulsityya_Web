    ...
    private void loadImages()
    {
        try
        {
            ...
            if ((dt.Rows.Count > 0))
            {
                ...
                BindAttachment(dt.Rows[0]["FSSAIimage"].ToString());
            }
    ...
    protected void BtnIdentity_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (isOk1 > 0)
            {
                ...
                if (j != 0)
                {
                    scrname = "<script>alert('FSSAI Certificate Upload successfully.!');location.replace('fasaidetail.aspx');</script>";
