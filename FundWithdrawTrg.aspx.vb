...
Imports System.IO
Imports System.Net
Partial Class App_UI_Application_Pages_FundWithdrawTrg
...
"ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]) ON [PRIMARY] ALTER TABLE [dbo].[Trnfundtransferbyadmin] ADD  DEFAULT (getdate()) FOR [Rectimestamp] ')"
    ...
    Private Sub FillDetail()
        ...
        If Session("compid") = "1081" Then
            ...
            End If
            sql = "exec sp_GetwithdrawalDetail_Update '" & idno & "','" & Wsessid & "','" & status & "'," & SelectType & "," & Legno & ",'N'"
        Else
            If txtMemId.Text <> "" Then
            ...
            If RbtStatus.SelectedValue = "N" Then
                Condition = Condition
            ElseIf RbtStatus.SelectedValue = "Z" Then
                Condition = Condition & " And a.Status<>'" & RbtStatus.SelectedValue & "'"
            Else
                Condition = Condition & " And a.Status='" & RbtStatus.SelectedValue & "'"
            End If
            sql = " Select 0 as legno,A.FormNo,A.ReqID,dbo.FormatDate(ReqDate,'dd-MMM-yyyy') As WithDrawDate,C.IdNo As MemberID,A.PayeeName,ReqAmount As Amount"
            ...
            sql &= "From Fundwithdrawls As A Left join M_BankMaster   As B On  A.BankID=B.BankCode And "
        ...
        If Session("compid") = "1081" Then
            If ddlsearchtype.SelectedValue = 2 Then
                GvData.Columns(27).Visible = True
            Else
                GvData.Columns(27).Visible = False
            End If
        Else
            GvData.Columns(27).Visible = False
        End If
    ...
    Protected Sub btnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExport.Click
        ...
        Try
            ...
            ElseIf (Session("CompID") = 1007) Then
                sql = "Select dbo.FormatDate(ReqDate,'dd-MMM-yyyy') As WithDrawDate,C.IdNo As MemberID,A.PayeeName,ReqAmount As Amount,"
                sql &= "C.IdNo +'-'+ A.PayeeName As [Payee Name With Id Number] "
                sql &= " ,Case When A.Status='P' Then 'PENDING' Else Case When A.Status='A' Then 'APPROVED' else 'REJECTED' end end As Status "
                sql &= " ,Case when a.Status='P' Then '' else A.Remark end As Remark,UserName as ProcessedBy,"
                sql &= "Replace(Convert(varchar,IssueDate,106),' ','-') + ' '+ CONVERT(varchar(15),CAST(IssueDate AS TIME),100) as ProcessedDate,A.WSessID as WeekNo"
                sql &= " ,Replace(Convert(Varchar,d.FrmDate,106),' ','-') as FromDate,Replace(Convert(varchar,d.ToDate,106),' ','-') as ToDate, "
                sql &= " ''''+A.AcNo as AcNo, A.IFSCode, B.BankName, A.BranchName, A.PanNo,PanStatus As PanStatus,BankStatus As BankStatus "
                sql &= "From Fundwithdrawls As A Left join M_BankMaster "
                sql &= "  As B On  A.BankID=B.BankCode And    B.RowStatus='Y' And B.ActiveStatus='Y' ,"
                sql &= " M_MemberMaster As C,V#PanBAnkStatus As KS,m_sESSNmASTER AS D "
                sql &= "Where A.wsESSId=D.sESSID And A.FormNo=C.FormNo and KS.formno  = C.formno " & Condition & "  Order by a.UserId, A.Formno"
            ElseIf Session("compid") = "1081" Then
                ...
                If DDlLagno.SelectedValue <> "0" Then
                    Legno = DDlLagno.SelectedValue
                End If
                sql = "exec sp_GetwithdrawalDetail_Update '" & idno & "','" & Wsessid & "','" & status & "'," & SelectType & "," & Legno & ",'Y'"
            Else
                sql = "Select dbo.FormatDate(ReqDate,'dd-MMM-yyyy') As WithDrawDate,C.IdNo As MemberID,A.PayeeName,ReqAmount As Amount"
                sql &= " ,Case When A.Status='P' Then 'PENDING' Else Case When A.Status='A' Then 'APPROVED' else 'REJECTED' end end As Status "
                sql &= " ,Case when a.Status='P' Then '' else A.Remark end As Remark,UserName as ProcessedBy,"
                sql &= "Replace(Convert(varchar,IssueDate,106),' ','-') + ' '+ CONVERT(varchar(15),CAST(IssueDate AS TIME),100) as ProcessedDate,A.WSessID as WeekNo"
                sql &= " ,Replace(Convert(Varchar,d.FrmDate,106),' ','-') as FromDate,Replace(Convert(varchar,d.ToDate,106),' ','-') as ToDate, "
                sql &= " ''''+A.AcNo as AcNo, A.IFSCode, B.BankName, A.BranchName, A.PanNo,C.Mobl as [Mobile No.],"
                sql &= "Case When IsPanVerified = 'N' Then 'PENDING' When IsPanVerified = 'R' Then 'REJECT' When IsPanVerified = 'Y' Then 'APPROVE' End As PanStatus,  "
                sql &= "Case When IsBankVerified = 'N' Then 'PENDING' When IsBankVerified = 'R' Then 'REJECT' When IsBankVerified = 'Y' Then 'APPROVE' End As BankStatus "
                sql &= "From Fundwithdrawls As A Left join M_BankMaster As B On  A.BankID=B.BankCode And    B.RowStatus='Y' And B.ActiveStatus='Y' ,"
                sql &= " M_MemberMaster As C, Kycverify As KS, m_sESSNmASTER AS D Where A.wsESSId=D.sESSID And A.FormNo=C.FormNo "
                sql &= "and KS.formno  = C.formno " & Condition & "  Order by a.UserId, A.Formno"
            End If
    ...
    Protected Sub BtnExportToCsv_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnExportToCsv.Click
        Try
            ...
            If (Session("CompID") = 1007) Then
                ...
                sql = "Select '' as CUSTOM_DETAILS1, dbo.FormatDate(ReqDate,'dd/MM/yyyy') as [Value Date], Case When BankCode = 22 then 'IFT' Else 'NEFT' end [Message Type], " & _
                      " ''''+'98120200000290' as [Debit Account No.], A.PayeeName as [Beneficiary Name],ReqAmount as [Payment Amount],c.IFSCode as [Beneficiary Bank Swift Code / IFSC Code], " & _
                      " ''''+A.AcNo as [Beneficiary Account No.],Case When BankCode = 22 then 'IFT' Else 'NEFT' end as [Transaction Type Code],'' as CUSTOM_DETAILS2," & _
                     " '' as CUSTOM_DETAILS3,'' as CUSTOM_DETAILS4,'' as CUSTOM_DETAILS5 , '' as CUSTOM_DETAILS6, '' as Remarks,'' as  [Purpose Of Payment] " & _
                     " From Fundwithdrawls As A Left join M_BankMaster " & _
                    "  As B On  A.BankID=B.BankCode And    B.RowStatus='Y' And B.ActiveStatus='Y' ," & _
                     " M_MemberMaster As C,m_sESSNmASTER AS D Where A.wsESSId=D.sESSID And A.FormNo=C.FormNo " & Condition & "   Order by ReqId"
    ...
    Protected Sub BtnApproveAll_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnApproveAll.Click
        ...
        If updateeffect > 0 Then
            ...
            If updateeffect <> 0 Then
                ScriptManager.RegisterStartupScript(Me, Me.[GetType](), "Key", "alert('" & cnt & "Withdrawal Approved Successfully.');location.replace('FundWithdrawTrg.aspx');", True)
            Else
            ...
            FillDetail()
        Else
            ScriptManager.RegisterStartupScript(Me, Me.[GetType](), "Key", "alert('Try After Some Time.');location.replace('FundWithdrawTrg.aspx');", True)
        End If
    End Sub
    Protected Sub btnRejectAll_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnRejectAll.Click
        ...
        If updateeffect > 0 Then
            ...
            Next
            If updateeffect <> 0 Then
                ScriptManager.RegisterStartupScript(Me, Me.[GetType](), "Key", "alert('" & cnt & " Withdrawal Rejected Successfully.');location.replace('FundWithdrawTrg.aspx');", True)
            Else
                scrname = "<SCRIPT language='javascript'>alert('Server Timeout, Try After Some Time.');" & "</SCRIPT>"
            End If
            Me.RegisterStartupScript("MyAlert", scrname)
