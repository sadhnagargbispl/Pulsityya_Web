...
Partial Class App_UI_Application_Pages_PaymentSummary
    ...
    Protected Sub BtnShow_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnShow.Click
        ...
        'sql = "select * from V#DailyPayoutDetail where 1=1 And (NetIncome>0 Or PrevBal>0 Or ClsBal>0) " & Condition & ""
        'dtData = New DataTable
    ...
    Public Sub BindData(ByVal PageIndex As Integer)
        ...
        Try
            Dim FromSessid As Date
            Dim ToSessid As Date
            ...
            Dim MaxAmt As String = "0.00"
            If txtMaxAmt.Text = "" Or Val(txtMaxAmt.Text = "0") Then
                MaxAmt = "0.00"
            Else
                MaxAmt = txtMaxAmt.Text
            End If
      
            If (Session("CompID") = "1103" Or Session("CompID") = "1100") Then
                ...
                prms(0) = New SqlParameter("@IDNo", Idno)
                prms(1) = New SqlParameter("@ToDate", FromSessid)
                prms(2) = New SqlParameter("@FromDate", ToSessid)
                ...
                prms(6) = New SqlParameter("@Netincome", MaxAmt)
                prms(7) = New SqlParameter("@RecordCount", ParameterDirection.Output)
                Ds = SqlHelper.ExecuteDataset(Session("MlmDatabase" & Session("CompID")), "sp_GetPayoutSummery_Rk", prms)
                GrdTotal.DataSource = Ds.Tables(0)
                GrdTotal.DataBind()
            Else
                Dim prms As SqlParameter() = New SqlParameter(7) {}
                prms(0) = New SqlParameter("@IDNo", Convert.ToString(Idno).ToLower())
                prms(1) = New SqlParameter("@ToDate", ToSessid)
                prms(2) = New SqlParameter("@FromDate", FromSessid)
                prms(3) = New SqlParameter("@PageIndex", PageIndex)
                prms(4) = New SqlParameter("@PageSize", Integer.Parse(ddlPageSize.SelectedValue))
                prms(5) = New SqlParameter("@IsExport", "N")
                prms(6) = New SqlParameter("@Netincome", MaxAmt)
                prms(7) = New SqlParameter("@RecordCount", ParameterDirection.Output)
                Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), "sp_GetPayoutSummery_RA", prms)
                GrdTotal.DataSource = Ds.Tables(0)
                GrdTotal.DataBind()
            End If
    ...
    Private Sub ExportExcel()
        ...
        Using wb As New XLWorkbook()
            ...
            Response.AddHeader("content-disposition", "attachment;filename=PayoutSummary.xlsx")
    ...
    Protected Sub btnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExport.Click
        Try
            ...
            Else
                ...
                Dim prms As SqlParameter() = New SqlParameter(7) {}
                prms(0) = New SqlParameter("@IDNo", Convert.ToString(Idno).ToLower())
                prms(1) = New SqlParameter("@FromDate", FromSessid)
                prms(2) = New SqlParameter("@ToDate", ToSessid)
                prms(3) = New SqlParameter("@PageIndex", 1)
                prms(4) = New SqlParameter("@PageSize", Integer.Parse(ddlPageSize.SelectedValue))
                prms(5) = New SqlParameter("@IsExport", "Y")
                prms(6) = New SqlParameter("@Netincome", MaxAmt)
                prms(6) = New SqlParameter("@RecordCount", ParameterDirection.Output)
                Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), "Sp_WeeklyPayoutSummary", prms)
                Session("GData1") = Ds.Tables(0)
                ExportExcel()
            End If
