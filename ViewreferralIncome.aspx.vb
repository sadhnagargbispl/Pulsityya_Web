...
Partial Class ViewreferralIncome
    ...
    Public Sub BindData(Optional ByVal SrchCond As String = "")
        Try
            ...
            If Session("compid") = "1090" Then
                sql = " Select A.SessID,replace(CONVERT(VARCHAR(11), CONVERT(DATE, CONVERT(VARCHAR(8), a.sessid), 112), 106),' ','-') As FrmDate,"
                sql &= "B.IDNo,B.MemFirstName,a.BV as Amount, A.Slab,A.Comm From MstRefIncome As A "
                sql &= " Inner join M_MemberMaster As B On A.FormNoDwn = B.FormNo Where "
                sql &= " A.FormNo = '" & Val(formno) & "' And A.SessID = '" & Request("Sessid") & "' and Comm<>0"
            Else
                sql = " Select A.SessID,replace(CONVERT(VARCHAR(11), CONVERT(DATE, CONVERT(VARCHAR(8), a.sessid), 112), 106),' ','-') As FrmDate,"
                sql &= "B.IDNo,B.MemFirstName,a.BV as Amount, A.Slab,A.Comm From MstRefIncome As A "
                sql &= " Inner join M_MemberMaster As B On A.FormNoDwn = B.FormNo Where "
                sql &= " A.FormNo = '" & Val(formno) & "' And A.DSessID = '" & Request("Sessid") & "' and Comm<>0"
            End If    
