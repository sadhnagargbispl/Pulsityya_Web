...
Partial Class App_UI_Application_Pages_CountryMaster
    ...
    Public Sub BindData()
        Try
            ...
            Dim sql As String = "Select CID,CountryCode,StdCode,Cast(CountryCode as varchar) as VCountryCode,CountryName,remarks,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status," & _
            " Case when ActiveStatus='N' then 'label label-danger' else 'label label-success' end as StatusClass From M_CountryMaster Where " + objDAL.activeCondition + " Order by CountryName"
            dtData = New DataTable
            dtData = objDAL.GetData(sql)
            GvData.DataSource = dtData
            GvData.DataBind()
            Session("GData") = dtData
            ViewState("WithDrawDate") = "CountryCode"
    ...
    Protected Sub DeleteGroup(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim Sql As String = "Update " + objDAL.tblStateMaster + " SET ActiveStatus='N',LastModified='De-Activated by " & Session("UserName") & " at " & DateTime.Now.ToString() & "' WHERE CID='" & Val(StateCode.ToString()) & "' AND RowStatus='Y'"
            ...
            If String.Equals(ddlSearchFields.SelectedItem.Text.ToLower(), "status") = True Then
                If String.IsNullOrEmpty(txtSearch.Text) = False Then
                    ...
                    Else
                        status = "Y"
                    End If
                    sql = "Select CID,CountryCode,StdCode,Cast(CountryCode as varchar) as VCountryCode,CountryName,remarks,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status From M_CountryMaster Where RowStatus='Y' AND ActiveStatus like '%" + status.ToString() + "%' Order by CountryName"
                End If
            Else
                sql = "Select CID,CountryCode,StdCode,Cast(CountryCode as varchar) as VCountryCode,CountryName,remarks,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status From M_CountryMaster Where " + objDAL.activeCondition + " AND " + ddlSearchFields.SelectedValue.ToString() + " like '%" + txtSearch.Text + "%' Order by CountryName"
            End If
            dtData = New DataTable
            dtData = objDAL.GetData(sql)
            GvData.DataSource = dtData
