...
Partial Class App_UI_Application_Pages_ComplaintType
    ...
    Public Sub BindData()
        Dim sql As String = "Select CTypeId,Cast(CTypeId as varchar) as VCTypeId,CType,a.Remarks,CASE WHEN a.ActiveStatus='Y' Then  " & _
        " 'Active' ELSE 'DeActive' END AS Status,B.UserName,a.ToUserEmail as UserEmail From " + objDAL.tblCTypeMaster + " as a," & _
        " M_UserMaster as b Where a.TouserId=b.UserId and b.ActiveStatus='Y' and b.RowStatus='Y' and a." + objDAL.activeCondition + " Order by CType"
            ...
            If String.Equals(ddlGroupFields.SelectedItem.Text.ToLower(), "status") = True Then
                If String.IsNullOrEmpty(txtSearch.Text) = False Then
                    ...
                    Else
                        status = "Y"
                    End If
                    sql = "Select CTypeId,Cast(CTypeId as varchar) as VCTypeId,CType,a.Remarks,CASE WHEN a.ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status,b.Username,a.ToUserEmail as UserEmail From " + objDAL.tblCTypeMaster + " as a,M_UserMaster as b Where a.ToUserId=b.UserId and b.RowStatus='Y' and b.ActiveStatus='Y' and  a.RowStatus='Y' AND a.ActiveStatus like '%" + status.ToString() + "%' Order by CType"
                End If
            Else
                sql = "Select CTypeId,Cast(CTypeId as varchar) as VCTypeId,CType,a.Remarks,CASE WHEN a.ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status,b.Username,a.ToUserEmail as UserEmail From " + objDAL.tblCTypeMaster + " as a,M_UserMaster as b Where a.ToUserId=b.UserId and b.RowStatus='Y' and b.ActiveStatus='Y' and  a. " + objDAL.activeCondition + " AND a." + ddlGroupFields.SelectedValue.ToString() + " like '%" + txtSearch.Text + "%' Order by CType"
            End If
            dtData = New DataTable
            dtData = objDAL.GetData(sql)
            GvData.DataSource = dtData
