...
Partial Class CategoryMaster
    ...
    Public Sub BindData()
        ...
        sql = "Select *,Cast(catid as varchar) as VKitId,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status From M_CatMaster Where RowStatus='Y'  Order by CatName"
    ...
    Protected Sub DeleteGroup(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim Sql As String = "Update M_CatMaster SET ActiveStatus='N',LastModified='De-Activated by " & Session("UserName") & " at " & DateTime.Now.ToString() & "' WHERE catid = '" & Val(KitId.ToString()) & "' AND RowStatus='Y'"
        Dim updateEffect As Integer = objDAL.UpdateData(Sql)
        If updateEffect <> 0 Then
            ScriptManager.RegisterStartupScript(Me, Me.[GetType](), "Key", "alert('Deleted Successfully.!');location.replace('CategoryMaster.aspx');", True)
