...
Partial Class App_UI_Application_Pages_Bonusincentivedetail
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        If Session("AStatus") = "OK" Then
            Session("PageName") = "Report  / Bonus Incentive Report"
        Else
    ...
    Public Sub BindSession()
        Try
            ...
            'ddlSession.DataBind()
            Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), "sp_GetRanks")
            ddlSession.DataSource = Ds.Tables(0)
            ddlSession.DataValueField = "RankId"
            ddlSession.DataTextField = "Rank"
            ddlSession.DataBind()
        Catch ex As Exception
        End Try
    End Sub
    ...
    Protected Sub BtnShow_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnShow.Click
        Try
            ...
            If CheckBox1.Checked = True Then
                ...
                Condition = txtMemId.Text
                'IncentiveDetailView(0, 1, Condition)
                IncentiveDetail(0, 1)
    ...
    Protected Sub ViewDetail(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            Lblidno.Text = DirectCast(GVRw.FindControl("LblPayoutDate"), Label).Text
            IncentiveDetailView(Val(LblSessionNo.Text), 1, Lblidno.Text)
            'IncentiveDetailView(0, 1, Lblidno.Text)
        Catch ex As Exception
    ...
    Private Sub IncentiveDetail(ByVal Sessid As Integer, ByVal PageIndex As Integer)
        Try
            ...
            Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), "sp_GetBONUSReportDetail", prms)
    ...
    Private Sub IncentiveDetailView(ByVal Sessid As Integer, ByVal PageIndex As Integer, ByVal Idno As String)
        Try
            ...
            If Sessid = 0 Then
                ...
                End If
            End If
            gvdataview.DataSource = Nothing
            gvdataview.DataBind()
            Dim prms As SqlParameter() = New SqlParameter(5) {}
            prms(0) = New SqlParameter("@IdNo", Convert.ToString(Idno).ToLower())
            prms(1) = New SqlParameter("@Sessid", Convert.ToInt32(Sessid))
            prms(2) = New SqlParameter("@PageIndex", PageIndex)
            prms(3) = New SqlParameter("@PageSize", 100)
            prms(4) = New SqlParameter("@IsExport", "N")
            prms(5) = New SqlParameter("@RecordCount", ParameterDirection.Output)
            Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), "sp_GetBONUSReportDetailView", prms)
            gvdataview.DataSource = Ds.Tables(0)
            gvdataview.DataBind()
            ...
            '    If gvdataview.Rows.Count > 0 Then
            ...
            If Ds.Tables(1).Rows(0)("RecordCount") > 0 Then
                For i As Integer = 1 To gvdataview.Columns.Count - 1
                    Dim tableCell As TableCell = gvdataview.HeaderRow.Cells(i)
                    Dim img As New Image()
                    img.ImageUrl = "~/Images/Uparrow.png"
                    tableCell.Controls.Add(New LiteralControl("&nbsp;"))
                    tableCell.Controls.Add(img)
    ...
    Protected Sub Gvdata_Sorting(ByVal sender As Object, ByVal e As GridViewSortEventArgs)
        ...
        End Try
    End Sub
    Protected Sub gvdataview_Sorting(ByVal sender As Object, ByVal e As GridViewSortEventArgs)
        Try
            ' If e.SortExpression = ViewState("PayoutDate").ToString() Then
            If ViewState("Sort_Order").ToString() = "ASC" Then
                RebindData(e.SortExpression, "DESC")
                For i As Integer = 0 To gvdataview.Columns.Count - 1
                    Dim lbText As String = "DESC"
                    ' Dim lbText As String = DirectCast(GvData.HeaderRow.Cells(i).Controls(0), LinkButton).Text
                    If lbText = ViewState("Sort_Order").ToString() Then
                        Dim tableCell As TableCell = gvdataview.HeaderRow.Cells(i)
    ...
    Protected Sub Page_Changed(ByVal sender As Object, ByVal e As EventArgs)
        Try
            ...
            If ddlSession.SelectedValue = "5000000" Then
                ...
                End If
                If Val(Lblidno.Text) <> 0 Then
                    idno = Val(Lblidno.Text)
                Else
                    idno = 0
                End If
            Else
    ...
    Protected Sub PageSize_Changed(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Try
            ...
            Me.IncentiveDetailView(sessid, 1, idno)
        Catch ex As Exception
    ...
    Private Sub ExportExcel()
        ...
        Using wb As New XLWorkbook()
            ...
            Response.AddHeader("content-disposition", "attachment;filename=BonusIncentiveDetail.xlsx")
