...
Partial Class CountryBankMaster
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        If Session("AStatus") = "OK" Then
            Session("PageName") = "Master / Bank Master"
        Else
    ...
    Public Sub BindData()
        Try
            ...
            sql = "Select  CountryName,BankCode,Cast(BankCode as varchar) as VBankCode,BankName,IFSCode" & _
            " ,AcNo,a.remarks,CASE WHEN a.ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status, " & _
            " Case when a.ActiveStatus='N' then 'label label-danger' else 'label label-success' end as StatusClass  " & _
            " From " + objDAL.tblBankMaster + " as a,M_CountryMaster as b Where a.CountryCode=b.Cid and b.ActiveStatus='Y' and " & _
            " a." + objDAL.activeCondition + " Order by BankName"
    ...
    Protected Sub DeleteGroup(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim Sql As String = "Update " + objDAL.tblBankMaster + " SET ActiveStatus='N',LastModified='De-Activated by " & Session("UserName") & " at " & DateTime.Now.ToString() & "' WHERE BankCode='" & Val(BankCode.ToString()) & "' AND RowStatus='Y'"
            ...
            If String.Equals(ddlSearchFields.SelectedItem.Text.ToLower(), "status") = True Then
                If String.IsNullOrEmpty(txtSearch.Text) = False Then
                    ...
                    Else
                        status = "Y"
                    End If
                    sql = "Select CountryName,BankCode,Cast(BankCode as varchar) as VBankCode,BankName,IFSCode,AcNo,a.remarks," & _
                    " CASE WHEN a.ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status, Case when a.ActiveStatus='N' " & _
                    " then 'label label-danger' else 'label label-success' end as StatusClass From " + objDAL.tblBankMaster + " as a,M_countryMaster as b" & _
                    " Where a." + objDAL.activeCondition + " and a.CountryCode=b.Cid  and b.RowStatus='Y' AND a.ActiveStatus like '%" + status.ToString() + "%' Order by BankName"
                End If
            Else
                sql = "Select CountryName,BankCode,Cast(BankCode as varchar) as VBankCode,BankName,IFSCode,AcNo,a.remarks," & _
                " CASE WHEN a.ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status," & _
                " Case when a.ActiveStatus='N' then 'label label-danger' else 'label label-success' end as StatusClass " & _
                " From " + objDAL.tblBankMaster + " as a,M_countryMaster as b" & _
                " Where a." + objDAL.activeCondition + "and a.CountryCode=b.Cid  and b.RowStatus='Y' AND  " + ddlSearchFields.SelectedValue.ToString() + " like '%" + txtSearch.Text + "%' Order by BankName"
            End If
            dtData = New DataTable
            dtData = objDAL.GetData(sql)
            GvData.DataSource = dtData
    ...
    Protected Sub btnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExport.Click
        ...
        Response.AddHeader("content-disposition", String.Format("attachment; filename={0}", "BankDetails.xls"))
