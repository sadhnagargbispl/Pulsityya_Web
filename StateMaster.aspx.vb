...
Partial Class App_UI_Application_Pages_StateMaster
    ...
    Public Sub BindData()
        Try
            ...
            Dim sql As String = " Select a.StateCode,Cast(a.StateCode as varchar) as VStateCode,a.statename,a.remarks,"
            sql &= " CASE WHEN a.ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status,"
            sql &= " Case when a.ActiveStatus='N' then 'label label-danger' else 'label label-success' end as StatusClass"
            sql &= " From M_StateDivMaster as a , M_CountryMAster as b  Where "
            sql &= " a.CountryCode =b.CID"
            ...
            If String.Equals(ddlSearchFields.SelectedItem.Text.ToLower(), "status") = True Then
                If String.IsNullOrEmpty(txtSearch.Text) = False Then
                    ...
                    Else
                        status = "Y"
                    End If
                    sql = "Select StateCode,Cast(StateCode as varchar) as VStateCode,statename,remarks,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status, Case when ActiveStatus='N' then 'label label-danger' else 'label label-success' end as StatusClass From " + objDAL.tblStateMaster + " Where RowStatus='Y' AND ActiveStatus like '%" + status.ToString() + "%' Order by StateName"
                End If
            Else
                sql = "Select StateCode,Cast(StateCode as varchar) as VStateCode,statename,remarks,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status, Case when ActiveStatus='N' then 'label label-danger' else 'label label-success' end as StatusClass From " + objDAL.tblStateMaster + " Where " + objDAL.activeCondition + " AND " + ddlSearchFields.SelectedValue.ToString() + " like '%" + txtSearch.Text + "%' Order by StateName"
            End If
            dtData = New DataTable
            dtData = objDAL.GetData(sql)
            GvData.DataSource = dtData
