...
Partial Class App_UI_Application_Pages_KitMaster
    ...
    Public Sub BindData()
        ...
        If Session("CompId") <> "1033" Then
            If Session("CompId") = "1010" Then
                sql = "Select *,'False' as PlanStatus,'' as Plan1,Cast(KitId as varchar) as VKitId,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status From " + objDAL.tblKitMaster + " Where " + objDAL.activeCondition + " and  OldKit<>'OLD' Order by kitid desc"

            Else
                sql = "Select 0 as CouponQty,0 as CouponAmount ,0 as RewardPoint,*,'False' as PlanStatus,'' as Plan1,Cast(KitId as varchar) as VKitId,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status,0 as WELLSMARTNOOFCOUPON,0 as WELLSMARTCOUPONAMOUNT From " + objDAL.tblKitMaster + " Where " + objDAL.activeCondition + " and  OldKit<>'OLD' Order by KitName"

            End If
                 Else
            sql = "Select 0 as CouponQty,0 as CouponAmount ,0 as RewardPoint,*,'True' as PlanStatus,Case when Plantype=0 then 'Choose Plan' when Plantype=1 then 'Plan A' when Plantype=2 then 'Plan B' else '' end as Plan1, Cast(KitId as varchar) as VKitId,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status,0 as WELLSMARTNOOFCOUPON,0 as WELLSMARTCOUPONAMOUNT From " + objDAL.tblKitMaster + " Where " + objDAL.activeCondition + " and  OldKit<>'OLD' Order by KitName"
        ...
        'GvData.Columns(13).HeaderText = If(Session("compid").ToString() = "1107", "EV", "BV")
    ...
    Protected Sub GvData_PreRender(ByVal sender As Object, ByVal e As EventArgs) Handles GvData.PreRender
        If GvData.Columns.Count > 13 Then
            If Session("compid") IsNot Nothing AndAlso Session("compid").ToString() = "1107" Then
                GvData.Columns(13).HeaderText = "EV"
            Else
                GvData.Columns(13).HeaderText = "BV"
            End If
        End If
    End Sub
    Protected Sub GvData_RowCreated(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles GvData.RowCreated
        If e.Row.RowType = DataControlRowType.Header Then
            For Each cell As TableCell In e.Row.Cells
                If cell.Text.Trim().ToUpper() = "BV" Then
                    cell.Text = If(Session("compid").ToString() = "1107", "EV", "BV")
                End If
            Next
        End If
    End Sub
