...
Partial Class ApprovePaymentReqs
    ...
    Public Sub BindData(Optional ByVal Condition As String = "")
        ...
        End If
        If DDlFilterOption.SelectedValue <> "A" Then
            If DDlFilterOption.SelectedValue = "O" Then
                Condition = Condition & " And a.Paymode = 'QrCode'"
            ElseIf DDlFilterOption.SelectedValue = "C" Then
                Requestno.Visible = True
                If txtreqno.Text <> "" Then
                    Condition = Condition & " And a.ReqNo like '" & txtreqno.Text & "%'"
                End If
        ...
        If Session("CompId") = 1074 Then
            ...
            sql &= "CASE WHEN ScannedFile='' THEN '' WHEN ScannedFile like 'http%' THEN ScannedFile "
            sql &= "Else 'https://login.cashlessbazar.in/images/UploadImage/'+ScannedFile END AS ScannedFile "
            sql &= " ,Case when ScannedFile='' then 'False' else 'True' end as ScannedFileStatus,ApproveRemark,'' as txnhash "
            sql &= "from WalletReq  as a  with(nolock),M_BankMaster as b  with(nolock), M_MemberMaster as c  with(nolock) "
            sql &= "where a.Formno=c.Formno and  a.BankId=b.BankCode and b.RowStatus='Y' " & Condition & " order by a.RectimeStamp desc"
        ElseIf Session("CompId") = 1093 Then
            ...
            sql &= "Case when IsApprove='N' then 'True' else 'False' end as EnableStatus,Amount,a.Remarks,"
            sql &= "CASE WHEN ScannedFile='' THEN '' WHEN ScannedFile like 'http%' THEN ScannedFile "
            sql &= "Else 'https://sollywood.in/images/UploadImage/'+ScannedFile END AS ScannedFile "
            sql &= " ,Case when ScannedFile='' then 'False' else 'True' end as ScannedFileStatus,ApproveRemark,'' as txnhash "
            sql &= "from WalletReq as a  with(nolock),M_BankMaster as b  with(nolock), M_MemberMaster as c  with(nolock) "
            sql &= "where a.Formno=c.Formno and  a.BankId=b.BankCode and b.RowStatus='Y' " & Condition & " order by a.RectimeStamp desc "
        ElseIf Session("CompId") = 1082 Then
            ...
            sql &= "Case when IsApprove='N' then 'True' else 'False' end as EnableStatus,Amount,a.Remarks,"
            sql &= "CASE WHEN ScannedFile='' THEN '' WHEN ScannedFile like 'http%' THEN ScannedFile "
            sql &= "Else 'https://cpanel.cbullsecommerce.com/images/UploadImage/'+ScannedFile END AS ScannedFile ,"
            sql &= "Case when ScannedFile='' then 'False' else 'True' end as ScannedFileStatus,ApproveRemark,'' as txnhash "
            sql &= "from WalletReq  as a  with(nolock),M_BankMaster as b  with(nolock), M_MemberMaster as c  with(nolock) "
            sql &= "where a.Formno=c.Formno and  a.BankId=b.BankCode and b.RowStatus='Y' " & Condition & " order by a.RectimeStamp desc "
        ElseIf Session("CompId") = 1092 Then
            ...
            sql &= "Case when IsApprove='N' then 'True' else 'False' end as EnableStatus,Amount,a.Remarks,"
            sql &= "CASE WHEN ScannedFile='' THEN '' WHEN ScannedFile like 'http%' THEN ScannedFile "
            sql &= "Else 'https://login.a7kart.com/images/UploadImage/'+ScannedFile END AS ScannedFile ,"
            sql &= "Case when ScannedFile='' then 'False' else 'True' end as ScannedFileStatus,ApproveRemark,'' as txnhash "
            sql &= "from WalletReq  as a  with(nolock),M_BankMaster as b  with(nolock), M_MemberMaster as c  with(nolock) "
            sql &= "where a.Formno=c.Formno and  a.BankId=b.BankCode and b.RowStatus='Y' " & Condition & " order by a.RectimeStamp desc "
        ElseIf Session("CompId") = 1072 Then
            ...
            sql &= "Case when IsApprove='N' then 'True' else 'False' end as EnableStatus,Amount,a.Remarks,"
            sql &= "CASE WHEN ScannedFile='' THEN '' WHEN ScannedFile like 'http%' THEN ScannedFile "
            sql &= "Else 'https://cpanel.luxewaytrends.com/images/UploadImage/'+ScannedFile END AS ScannedFile "
            sql &= " ,Case when ScannedFile='' then 'False' else 'True' end as ScannedFileStatus,ApproveRemark,'' as txnhash "
            sql &= "from WalletReq as a  with(nolock),M_BankMaster as b  with(nolock), M_MemberMaster as c  with(nolock) "
            sql &= "where a.Formno=c.Formno and  a.BankId=b.BankCode and b.RowStatus='Y' " & Condition & " order by a.RectimeStamp desc "
        ElseIf Session("CompId") = 1007 Then
            ...
            sql &= " ,Case when ScannedFile='' then 'False' else 'True' end as ScannedFileStatus,ApproveRemark,'' as txnhash "
            sql &= "from WalletReq as a  with(nolock),M_BankMaster as b  with(nolock), M_MemberMaster as c  with(nolock) "
            sql &= "where  ForType = 'C' And a.Formno=c.Formno and  a.BankId=b.BankCode and b.RowStatus='Y' " & Condition & " order by a.RectimeStamp desc "
        ElseIf Session("CompId") = 1089 Then
            ...
            sql &= "Case when IsApprove='N' then 'Pending' when IsApprove='Y' then 'Approve' else 'Rejected' end as status,"
            sql &= "Case when IsApprove='N' then 'True' else 'False' end as EnableStatus,Amount,a.Remarks,"
            sql &= "Case when ScannedFile='' then '' else 'https://megamart.ai/img/UploadImage/'+ ScannedFile end as ScannedFile "
            sql &= " ,Case when ScannedFile='' then 'False' else 'True' end as ScannedFileStatus,ApproveRemark,txnhash "
            sql &= "from WalletReq as a with(nolock),M_BankMaster as b with(nolock), M_MemberMaster as c with(nolock) "
            sql &= " where a.Formno=c.Formno and  a.BankId=b.BankCode and b.RowStatus='Y' " & Condition & " order by a.RectimeStamp desc "
        ElseIf Session("compid") = "1102" Then
            sql = " select a.Formno,c.Idno,(c.MemFirstName+ ' '+c.MemlastName) as Membername,Email as Email ,Mobl as Mobileno,ReqNo,'' as Actype,"
            sql &= " Replace(Convert(Varchar,a.RectimeStamp,106),' ','-')+ ' '+ CONVERT(varchar(15), CAST(a.RectimeStamp AS TIME),100) as ReqDate,"
            sql &= "a.PayMode,a.Chqno,Replace(Convert(Varchar,a.ChqDate,106),' ','-') as ChequeDate,b.BankName,a.Branchname,"
            sql &= "Case when IsApprove='N' then 'Pending' when IsApprove='Y' then 'Approve' else 'Rejected' end as status,"
            sql &= "Case when IsApprove='N' then 'True' else 'False' end as EnableStatus,Amount,a.Remarks,"
            sql &= "Case when ScannedFile='' then '' else 'https://cpanel.makeandgrowth.com/images/UploadImage/" & Session("CompId") & "/'+ ScannedFile end as ScannedFile "
            sql &= " ,Case when ScannedFile='' then 'False' else 'True' end as ScannedFileStatus,ApproveRemark,'' as txnhash "
            sql &= "from WalletReq as a with(nolock),M_BankMaster as b with(nolock), M_MemberMaster as c with(nolock) "
            sql &= " where a.Formno=c.Formno and  a.BankId=b.BankCode and b.RowStatus='Y' " & Condition & " order by a.RectimeStamp desc "
        ElseIf Session("compid") = "1105" Then
            ...
            sql &= "Case when IsApprove='N' then 'Pending' when IsApprove='Y' then 'Approve' else 'Rejected' end as status,"
            sql &= "Case when IsApprove='N' then 'True' else 'False' end as EnableStatus,Amount,a.Remarks,"
            sql &= "Case when ScannedFile='' then '' else 'https://cpanel.powerlifewellness.com/images/UploadImage/" & Session("CompId") & "/'+ ScannedFile end as ScannedFile "
            sql &= " ,Case when ScannedFile='' then 'False' else 'True' end as ScannedFileStatus,ApproveRemark,'' as txnhash "
            sql &= "from WalletReq as a with(nolock),M_BankMaster as b with(nolock), M_MemberMaster as c with(nolock) "
            sql &= " where a.Formno=c.Formno and  a.BankId=b.BankCode and b.RowStatus='Y' " & Condition & " order by a.RectimeStamp desc "
        Else
                ...
                If (Val(Dt1.Rows(0)("Cnt")) = 0) Then
                    If AprvType = "Y" Then
                        ...
                        ElseIf Session("compid") = 1089 Then
                            ...
                            sql &= "'Amount Added by Payment  Req.No.:" & lblReqno.Text & ".','Req/" & lblReqno.Text & "','M','C',Convert(Varchar,Getdate(),112),"
                        ...
                        ElseIf Session("CompId") = 1084 Then
                            ...
                            End If
                        Else
                            If Session("CompId") = 1072 Or Session("CompId") = 1073 Or Session("CompId") = 1077 Or Session("CompId") = 1078 Or Session("CompId") = 1079 Or Session("CompId") = 1082 Or Session("CompId") = 1083 Or Session("CompId") = 1092 Or Session("CompID") = 1093 Or Session("compID") = 1101 Then
                                Remark = " Approve Payment Request On ReqNo:" & lblReqno.Text & " for Idno:" & lblIdno.Text & ""
                                sql = sql & "INSERT INTO TrnVoucher(VoucherNo,VoucherDate,DrTo,CrTo,Amount,Narration,RefNo,AcType,VTYpe,SessID,WSessID) SELECT ISNULL(Max(VoucherNo)+1,1001),'" & Format(Now, "dd-MMM-yyyy") & "'," & _
                            "'0','" & lbl.Text & "'," & Val(LblAmount.Text) & ",'Amount Added by Payment  Req.No.:" & lblReqno.Text & ".','Req/" & lblReqno.Text & "','S','C',Convert(Varchar,Getdate(),112),'" & Session("CurrentSessn") & "' FROM TrnVoucher Where Narration Not in ('Amount Added by Payment  Req.No.:" & lblReqno.Text & ".');"
                            Else
...
"<span style=""color: #0099FF; font-weight: bold;"">Regards,</span><br />" & _
    ...
    Protected Sub btnApprove_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnApprove.Click
        If Session("compid") = 1066 Or Session("compid") = 1072 Or Session("compid") = 1073 Or Session("compid") = 1074 Or Session("compid") = 1075 Or Session("compid") = 1077 Or Session("compid") = 1078 Or Session("compid") = 1079 Or Session("compid") = 1082 Or Session("compid") = 1092 Or Session("CompID") = 1093 Then
        ...
        End If
        AprvAction("Y", "A")
        If Session("compid") = 1066 Or Session("compid") = 1072 Or Session("compid") = 1073 Or Session("compid") = 1074 Or Session("compid") = 1075 Or Session("compid") = 1082 Or Session("compid") = 1092 Then
    ...
    Protected Sub btnReject_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnReject.Click
        If Session("compid") = 1066 Or Session("compid") = 1072 Or Session("compid") = 1073 Or Session("compid") = 1074 Or Session("compid") = 1075 Or Session("compid") = 1078 Or Session("compid") = 1082 Or Session("compid") = 1092 Or Session("CompID") = 1093 Then
    ...
    Protected Sub btnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExport.Click
        Try
            ...
            Dim sql As String = ""
            If Session("compid") = "1089" Then
                sql = " select ReqNo,c.Idno as [ID No.],(c.MemFirstName+ ' '+c.MemlastName) as [Member Name],Mobl as [Mobile No],"
                sql &= "Replace(Convert(Varchar,a.RectimeStamp,106),' ','-')+ ' '+   CONVERT(varchar(15),CAST(a.RectimeStamp AS TIME),100) as [Request Date],"
                sql &= "a.PayMode as [Payment Mode],''''+a.Chqno as [Cheque/ TransactionNo],Replace(Convert(Varchar,a.ChqDate,106),' ','-') as [Cheque/ Transaction Date],"
                sql &= "b.BankName as [Bank Name],a.Branchname as [Branch Name],Amount,a.Remarks as [Member Remark], "
                sql &= " Case when IsApprove='N' then 'Pending' when IsApprove='Y' then 'Approve' else 'Rejected' end as status,ApproveRemark as [User Remark],txnhash "
                sql &= "from WalletReq as a  with(nolock),M_BankMaster as b  with(nolock), M_MemberMaster as c  with(nolock) "
                sql &= "where a.Formno = c.Formno And a.BankId=b.BankCode and b.RowStatus='Y' " & Condition & " order by ReqNo "
            Else
                sql = " select ReqNo,c.Idno as [ID No.],(c.MemFirstName+ ' '+c.MemlastName) as [Member Name],Mobl as [Mobile No],"
                sql &= "Replace(Convert(Varchar,a.RectimeStamp,106),' ','-')+ ' '+   CONVERT(varchar(15),CAST(a.RectimeStamp AS TIME),100) as [Request Date],"
                sql &= "a.PayMode as [Payment Mode],''''+a.Chqno as [Cheque/ TransactionNo],Replace(Convert(Varchar,a.ChqDate,106),' ','-') as [Cheque/ Transaction Date],"
                sql &= "b.BankName as [Bank Name],a.Branchname as [Branch Name],Amount,a.Remarks as [Member Remark], "
                sql &= " Case when IsApprove='N' then 'Pending' when IsApprove='Y' then 'Approve' else 'Rejected' end as status,ApproveRemark as [User Remark],'' as txnhash "
                sql &= "from WalletReq as a  with(nolock),M_BankMaster as b  with(nolock), M_MemberMaster as c  with(nolock) "
                sql &= "where a.Formno = c.Formno And a.BankId=b.BankCode and b.RowStatus='Y' " & Condition & " order by ReqNo "
            End If
    ...
    '    End Try
    'End Sub
    Protected Sub DDlFilterOption_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DDlFilterOption.TextChanged
        BindData()
    End Sub
End Class
