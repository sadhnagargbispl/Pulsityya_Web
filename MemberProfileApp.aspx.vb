...
Partial Class App_UI_Application_Pages_MemberProfileApp
    ...
    Public Sub BindData(ByVal PageIndex As Integer)
        ...
        Try
            ...
            'prms(9) = New SqlParameter("@AppWeb", Convert.ToString(ddlRegfrom.SelectedValue))
            'prms(10) = New SqlParameter("@RecordCount", ParameterDirection.Output)
            'Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), "sp_GetmemberProfileDetailApp", prms)

            Dim str As String = "sp_GetmemberProfileDetailApp '" & Convert.ToString(Idno).ToLower() & "','" & KitName & "',"
            str &= " '" & BankName & "','" & CmbType.SelectedValue & "','" & startDate & "','" & endDate & "',"
            str &= " " & PageIndex & "," & ddlPageSize.SelectedValue & ",'N','" & ddlRegfrom.SelectedValue & "',0"
    ...
    Protected Sub btnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExport.Click
        ...
        Try
            ...
            'prms(9) = New SqlParameter("@AppWeb", ddlRegfrom.SelectedValue)
            'prms(10) = New SqlParameter("@RecordCount", ParameterDirection.Output)
            ...
            Dim str As String = "sp_GetmemberProfileDetailApp '" & Convert.ToString(Idno).ToLower() & "','" & KitName & "',"
            str &= " '" & BankName & "','" & CmbType.SelectedValue & "','" & startDate & "','" & endDate & "',"
            str &= " 1," & ddlPageSize.SelectedValue & ",'Y','" & ddlRegfrom.SelectedValue & "',0"
            Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), CommandType.Text, str)
