    ...
    Public Sub BindData(Optional ByVal Condition As String = "")
        Try
            ...
            If DDlVerify.SelectedValue <> "S" Then
                Condition = Condition & " And c.IsFSSAIVerified='" & DDlVerify.SelectedValue & "'  "
            End If
            ...
            If txtStartDate.Text <> "" Then
                Condition = Condition & " And  Cast(Convert(varchar,c.FSSAIVerifyDate,106) as DateTime)>='" & txtStartDate.Text & "'  "
            End If
            If txtEndDate.Text <> "" Then
                Condition = Condition & " And  Cast(Convert(varchar,c.FSSAIVerifyDate,106) as DateTime)<='" & txtEndDate.Text & "'  "
            End If
            'End If
            Dim sql As String = ""
            ...
            "FSSAINo as GSTNo,CASE WHEN c.IsFSSAIVerified='Y' THEN 'Verified' WHEN c.IsFSSAIVerified='R' THEN 'Rejected' ELSE 'Verification Due' END AS GStVerf, " & _
            "case when c.FSSAIimage<>'' then Replace(CONVERT(varchar,c.FSSAIImageDate ,106),' ','-') else '' end as GstProofDate," & _
            "CASE WHEN c.FSSAIimage <>'' Then c.FSSAIimage ELSE '' END GstProofStatus,    '' as Link,case when c.IsFSSAIVerified='Y' then 'False' else 'True' end as EnableStatus, " & _
            "case when FSSAIimage like '%.pdf' or FSSAIimage like '%.PDF' then '' else FSSAIimage end as PDfLink,Case when c.IsFSSAIVerified='N' then '' else Replace(CONVERT(varchar,c.FSSAIVerifyDate ,106),' ','-') end as GSTVerifyDate," & _
            "Case when c.IsFSSAIVerified='N' then '' else c.FSSAIRemark end as VerifyRemark, Isnull(e.UserName,' ') As VerifyBy,Isnull(f.Reason, '') As RejectReason " & _
                    " From M_MemberMaster as a,M_StatedivMaster as b, M_BAnkMaster as g " & _
                            "  ,KycVerify as c Left Join  M_KycReject as f On c.FSSAIRejectId=f.Kid Left Join " & _
                            " M_UserMaster as e On c.FSSAIUserId=e.Userid and e.Rowstatus='Y' ,M_IdTypeMaster as d where " & _
                            ...
                            " and FSSAIimage <>'' " & Condition & " Order by FSSAIImageDate DESC"
    ...
    Protected Sub BtnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnExport.Click
        Try
            ...
            sql = " select a.IDNo,RTRIM(a.MemFirstName +' ' +a.MemLastName) as MemberName,Replace(CONVERT(varchar,a.Doj,106),' ','-')as Doj," & _
                           " Case when a.Activestatus='Y' then Replace(CONVERT(varchar,a.UpgradeDate,106),' ','-') else '' End as ActivationDate,''''+FSSAINo as FSSAINo, " & _
                           " CASE WHEN c.IsFSSAIVerified='Y' THEN 'Verified' when c.IsFSSAIVerified='R' then 'Rejected' Else 'Verification Due' END AS Verification," & _
                           " Case when c.IsFSSAIVerified='Y' then Replace(CONVERT(varchar,c.FSSAIVerifyDate ,106),' ','-') else '' end  as VerifyDate " & _
                           ", Isnull(e.Username,'')VerifyBy," & _
                           " Case when c.IsFSSAIVerified='R' then c.FSSAIRemark else '' end as RejectRemark,Isnull(f.Reason, '')As RejectReason" & _
                           " From M_MemberMaster as a,M_StatedivMaster as b, M_BAnkMaster as g ,KycVerify as c Left Join  M_KycReject as f On c.FSSAIRejectId=f.Kid Left Join M_UserMaster as e On c.FSSAIUserId=e.Userid and e.Rowstatus='Y' " & _
                           ...
                           " " & Condition & " and FSSAIimage <>'' Order by ActivationDate DESC"
        ...
        Try
            ...
            For Each Gvr As GridViewRow In GvData.Rows
                ...
                If Chk.Checked = True Then
                    ...
                    strsql = " select Count(formno) as cnt from KycVerify where IsFSSAIVerified in ('Y','R') and Formno='" & Val(lbl.Text) & "' "
                    ...
                    If Convert.ToInt32(dtcheck.Rows(0)("Cnt") = 0) Then
                        ...
                        str = str & "; Update KycVerify Set IsFSSAI = 'Y',IsFSSAIVerified = 'Y',FSSAIVerifyDate = getdate(),FSSAIUserId = '" & Val(Session("Userid")) & "'"
                        str = str & " where IsFSSAIVerified <> 'Y' AND formno='" & lbl.Text & "';"
        ...
        Try
            ...
            For Each Gvr As GridViewRow In GvData.Rows
                ...
                If Chk.Checked = True Then
                    Dim strsql As String = ""
                    Dim dtcheck As DataTable = New DataTable()
                    strsql = " select Count(formno) as cnt from KycVerify where IsFSSAIVerified = 'R' and Formno='" & Val(lbl.Text) & "' "
                    dtcheck = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), CommandType.Text, strsql).Tables(0)
                    If Convert.ToInt32(dtcheck.Rows(0)("Cnt") = 0) Then
                        Remark = "KYC  UnVerify of IdNo:" & LblIdno.Text & ""
                        str = str & "; Update KycVerify Set IsFSSAI = 'R',IsFSSAIVerified = 'R', FSSAIVerifyDate = getdate(),FSSAIUserId = '" & Val(Session("Userid")) & "', FSSAIRemark='" & TxtARemark.Text & "',FSSAIRejectId='" & DDlREason.SelectedValue & "'  " & _
                         " where IsFSSAIVerified <> 'R'  and  formno='" & lbl.Text & "';"
