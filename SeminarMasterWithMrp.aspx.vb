...
Partial Class App_UI_Application_Pages_SeminarMasterWithMrp
    ...
    Public Sub BindData(Optional ByVal Condition As String = "")
        Dim sql As String = ""
        sql = " Select ROW_NUMBER() Over(Order by AID DESC) As SNo,"
        sql &= " CASE WHEN a.UserType='A' THEN 'Admin' Else M.IDNO+ ' ('+M.MemFirstName+')'  END as UserID,MeetingID,"
        sql &= " Cast(MeetingID as varchar) as VMeetingID,dbo.FormatDate(MeetingTime,'dd-MM-yyyy') As Date,CONVERT(varchar(15),CAST(MeetingTime AS TIME),100) As Time,"
        sql &= " CityName,DistrictName,B.StateName,A.Address as Venue,Program,Speaker,A.ContactPerson ,A.ContactNo ,A.Remarks As Description,"
        sql &= " CASE WHEN A.ActiveStatus='Y' THEN 'Active' ELSE 'DeActive' END AS Status,"
        sql &= " Case when A.ActiveStatus='N' then 'label label-danger' else 'label label-success' end as StatusClass,MRP,Person"
        sql &= " From M_StateDivMaster As B,M_MeetingMaster As A LEFT JOIN M_MemberMaster as M ON A.UserID=M.FormNo"
        sql &= " Where A.StateCode=B.StateCode and B.RowStatus='Y' " & Condition & " ORDER BY MeetingTime DESC"
