...
Partial Class KYCVERIFYWITHPDF
...
" case when c.PanImg<>'' then Replace(CONVERT(varchar,c.PANImgDate ,106),' ','-') else '' end  as PanProofDate, CASE WHEN c.PanImg <>'' Then c.PanImg ELSE '' END PanproofStatus," & _
       " Case when c.IsPanverified='N' then '' else Replace(CONVERT(varchar,c.PanVerifyDate ,106),' ','-') end as PanVerifyDate,CASE WHEN ISNULL(FrontSideForm,'') <> '' THEN FrontSideForm ELSE '' END AS FrontSideForm " & _
