...
function Node(id, pid, category, doj, UpgradeDate, name, nodename, upliner, sponsor, leftbv, rightbv, DirctLeftActive, DirectRightActive, url, title, target, icon, iconOpen, open,CurrentMonRightRbv, AmRightRbv,rank) {
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;
...
dTree.prototype.add = function(id, pid, category, doj, UpgradeDate, name, nodename, upliner, sponsor, leftbv, rightbv, DirctLeftActive, DirectRightActive, url, title, target, icon, iconOpen, open ,CurrentMonRightRbv, AmRightRbv,rank) {
this.aNodes[this.aNodes.length] = new Node(id, pid, category, doj, UpgradeDate, name, nodename, upliner, sponsor, leftbv, rightbv, DirctLeftActive, DirectRightActive, url, title, target, icon, iconOpen, open ,CurrentMonRightRbv, AmRightRbv,rank);
};
...
//            strTable += '</tr>';
            ...
            {
             strTable += '<tr>';
            strTable += '<td bgcolor=#277AB1><font color=#FFFFFF FACE=Verdana SIZE=1><strong>Rank</strong></font></td>';
            strTable += '<td><font FACE=Verdana SIZE=1>' + node.rank + '&nbsp;</td>';
            strTable += '</tr>';
            }
        }
        else {
    ...
    str += "</td></tr><tr><td valign=top align=center style='white-space:nowrap; color:blue;'>";
    if (node.url) {
        str += '<a id="s' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '" href="' + node.url + '" style="color:blue;"';
