...
function Node(id, pid, category, doj, UpgradeDate, name, nodename, upliner, sponsor, leftbv, rightbv, DirctLeftActive, DirectRightActive, url, title, target, icon, iconOpen, open, CurrentMonRightRbv, AmRightRbv) {
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;
...
dTree.prototype.add = function(id, pid, category, doj, UpgradeDate, name, nodename, upliner, sponsor, leftbv, rightbv, DirctLeftActive, DirectRightActive, url, title, target, icon, iconOpen, open, CurrentMonRightRbv, AmRightRbv) {
this.aNodes[this.aNodes.length] = new Node(id, pid, category, doj, UpgradeDate, name, nodename, upliner, sponsor, leftbv, rightbv, DirctLeftActive, DirectRightActive, url, title, target, icon, iconOpen, open, CurrentMonRightRbv, AmRightRbv);
};
...
dTree.prototype.node = function(node, nodeId) {
    ...
    if (this.config.useIcons) {
        ...
        if (node.icon != 'img/gray.png') {
            ...
            strTable += '</tr>';
            strTable += '<td bgcolor=#277AB1><font color=#FFFFFF FACE=Verdana SIZE=1><strong>Current PV </strong></font></td>';
            strTable += '<td><font FACE=Verdana SIZE=1>' + node.CurrentMonRightRbv + '&nbsp;</td>';
            strTable += '</tr>';
            strTable += '<td bgcolor=#277AB1><font color=#FFFFFF FACE=Verdana SIZE=1><strong>Group Total PV </strong></font></td>';
            strTable += '<td><font FACE=Verdana SIZE=1>' + node.AmRightRbv + '&nbsp;</td>';
            strTable += '</tr>';
        }
        else {
            strTable += '<tr>';
