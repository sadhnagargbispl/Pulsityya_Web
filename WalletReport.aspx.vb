...
Partial Class App_UI_Application_Pages_PairDirectReport
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        If Session("AStatus") = "OK" Then
            Session("PageName") = "Wallet / Wallet Balance Report"
        Else
    ...
    Protected Sub GvData_PageIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles GvData.PageIndexChanging
        ...
        GvData.DataSource = Session("WalletData")
        GvData.DataBind()
    End Sub
    ...
    Protected Sub btnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExport.Click
        Try
            ...
            If txtStartDate.Text = "" Then
                txtStartDate.Text = Session("CompDate")
            Else
                txtStartDate.Text = txtStartDate.Text
            End If
            If txtEndDate.Text = "" Then
                txtEndDate.Text = Format(Date.Now, "dd-MMM-yyyy")
            Else
            ...
            prms(2) = New SqlParameter("@PageSize", Integer.Parse(ddlPageSize.SelectedValue))
            ...
            Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), "sp_GetWalletBalanceDetail", prms)
    ...
    Private Sub ExportExcel()
        Dim dt As DataTable = Session("WalletData1")
    ...
    Private Sub RebindData(ByVal sColimnName As String, ByVal sSortOrder As String)
        Try
            Dim dt As DataTable = CType(Session("WalletData"), DataTable)
            dt.DefaultView.Sort = sColimnName + " " + sSortOrder
            GvData.DataSource = dt
            GvData.DataBind()
            ViewState("SNo") = sColimnName
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        BindData(1)
    End Sub
    Public Sub CreateOrAlter_sp_GetWalletBalanceDetail()
        Dim sql As String = _
"CREATE OR ALTER PROCEDURE dbo.sp_GetWalletBalanceDetail" & _
"( " & _
" @IDNo NVARCHAR(200), " & _
" @PageIndex INT = 1, " & _
" @PageSize INT = 10, " & _
...
" @FromDate DATETIME, " & _
" @ToDate DATETIME, " & _
" @RecordCount INT OUTPUT " & _
") AS BEGIN SET NOCOUNT ON; " & _
"DECLARE @SQL NVARCHAR(MAX)='', @Cols NVARCHAR(MAX)=''; " & _
"SELECT @Cols=@Cols+' ,SUM(CASE WHEN vt.actype='''+CAST(actype AS VARCHAR)+''' AND t.VType=''C'' THEN t.Amount ELSE 0 END) AS ['+WalletName+'-Credit]' " & _
"+' ,SUM(CASE WHEN vt.actype='''+CAST(actype AS VARCHAR)+''' AND t.VType=''D'' THEN t.Amount ELSE 0 END) AS ['+WalletName+'-Debit]' " & _
"+' ,SUM(CASE WHEN vt.actype='''+CAST(actype AS VARCHAR)+''' THEN CASE WHEN t.VType=''C'' THEN t.Amount ELSE -t.Amount END ELSE 0 END) AS ['+WalletName+'-Balance]' " & _
"FROM VoucherType WHERE ActiveStatus='Y'; " & _
"IF OBJECT_ID('tempdb..##Results') IS NOT NULL DROP TABLE ##Results; " & _
"SET @SQL='SELECT ROW_NUMBER() OVER(ORDER BY b.IdNo) AS SNo, b.IdNo AS [Member ID], " & _
"b.MemFirstname+'' ''+b.MemLastName AS [Member Name] '+@Cols+' INTO ##Results " & _
"FROM TrnVoucher t INNER JOIN VoucherType vt ON vt.Actype=t.Actype " & _
"INNER JOIN M_MemberMaster b ON b.Formno=CASE WHEN t.CrTo<>0 THEN t.CrTo ELSE t.DrTo END " & _
"WHERE CAST(t.VoucherDate AS DATE) BETWEEN '''+CONVERT(VARCHAR,@FromDate,23)+''' AND '''+CONVERT(VARCHAR,@ToDate,23)+''' " & _
"AND ('''+@IDNo+'''=''0'' OR b.IdNo='''+@IDNo+''') " & _
"GROUP BY b.IdNo,b.MemFirstname,b.MemLastName'; " & _
"EXEC(@SQL); " & _
"IF @IsExport='N' BEGIN " & _
"SELECT * FROM ##Results WHERE SNo BETWEEN (@PageIndex-1)*@PageSize+1 AND (@PageIndex*@PageSize); " & _
"SELECT @RecordCount=COUNT(*) FROM ##Results; END " & _
"ELSE BEGIN SELECT * FROM ##Results; END SELECT @RecordCount as RecordCount; " & _
"DROP TABLE ##Results; END"
        Using c As New SqlConnection(HttpContext.Current.Session("MlmDatabase" & Session("CompID")).ToString())
            c.Open()
