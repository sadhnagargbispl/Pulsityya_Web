...
Partial Class App_UI_Application_Pages_GroupMaster
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        If Session("AStatus") = "OK" And Session("UserPermission") = 1 Then
            Session("PageName") = "User / New Department"
        Else
    ...
    Public Sub BindData()
        Dim sql As String = "Select GroupId,Cast(GroupId as varchar) as VGroupId,GroupName,Remarks,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status From " + objDAL.tblUserGrpMaster + " Where rowStatus='Y' Order by GroupName"
    ...
    Protected Sub DeleteGroup(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim Sql As String = "Update " + objDAL.tblUserGrpMaster + " SET ActiveStatus='N',LastModified='De-Activated by " & Session("UserName") & " at " & DateTime.Now.ToString() & "' WHERE GroupId='" & GrpID & "' AND RowStatus='Y'"
    ...
    Protected Sub imgSearch_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgSearch.Click
        ...
        ElseIf ddlGroupFields.SelectedValue = "showall" Then
            BindData()
            ...
            If String.Equals(ddlGroupFields.SelectedItem.Text.ToLower(), "status") = True Then
                If String.IsNullOrEmpty(txtSearch.Text) = False Then
                    ...
                    Else
                        status = "Y"
                    End If
                    sql = "Select GroupId,Cast(GroupId as varchar) as VGroupId,GroupName,Remarks,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status From " + objDAL.tblUserGrpMaster + " Where RowStatus='Y' AND ActiveStatus like '%" + status.ToString() + "%' Order by GroupName"
                End If
            Else
                sql = "Select GroupId,Cast(GroupId as varchar) as VGroupId,GroupName,Remarks,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status From " + objDAL.tblUserGrpMaster + " Where rowStatus='Y' AND " + ddlGroupFields.SelectedValue.ToString() + " like '%" + txtSearch.Text + "%' Order by GroupName"
            End If
            dtData = New DataTable
            dtData = objDAL.GetData(sql)
            GvData.DataSource = dtData
