...
Partial Class CommunityTree
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        Dim depthlevel As String = ""
        If txtDeptlevel.Text = "" Then
            depthlevel = 3
        Else
            depthlevel = txtDeptlevel.Text
        End If
        If TxtReEntry.Text = "" Then
            rentry = 1
        Else
            rentry = Val(TxtReEntry.Text)
        End If
        TreeFrame.Attributes.Item("src") = "treeCommunity.aspx?DownLineFormNo=" & Val(formno.ToString()) & "&deptlevel=" & Val(depthlevel.ToString()) & "&entry=" & Val(rentry)
    End Sub
            ...
            If Request.QueryString("key") <> "" And Not Request.QueryString("key") Is Nothing Then
                If Not Page.IsPostBack Then
                    ...
                    If formno <> "" Then
                        Dim level As String
                        level = 4
                        TreeFrame.Attributes.Item("src") = "treeCommunity.aspx?DownLineFormNo=" & Val(formno.ToString()) & "&deptlevel=" & Val(level.ToString()) & "&entry=" & Val(rentry)
                    End If
                End If
            End If
        End If
