Imports System.Data
Partial Class App_UI_Application_Pages_UnBlock
...
#End Region
    ...
    Protected Sub BtnBlock_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnBlock.Click
        ...
        If Session("CompId") = 1091 Then
            Sql = "Exec Sp_SaveBolcdata '" & Trim(TxtFormNo.Text) & "'"
            Dim updateEffect1 As Integer = 0
            updateEffect1 = objDAL.UpdateData(Sql)
        End If
        If rdblistChoice.SelectedValue = "single" Then
       ...
       "('" & Val(Session("UserID")) & "','" & Session("UserName") & "','UnBlock ','UnBlock Id','" & Remark & "',Getdate(),'" & TxtFormNo.Text & "')"
        ...
        Else
            ...
            If Session("") = 1057 Then
                ...
                Sql = "UPDATE M_MemberMaster SET IsBlock='N',BlockDate=GETDATE(),BlockRemark='" & txremarks.Text & "' FROM M_MemberMaster as a,M_MemTreeRelation as b WHERE b.FormNodwn=a.FormNo AND b.FormNo='" & Trim(TxtFormNo.Text) & "'  AND a.IsBlock='Y'"
                Sql = Sql & "; insert into UserHistory(UserId,UserName,PageName,Activity,ModifiedFlds,RecTimeStamp,MemberId)Values" & _
