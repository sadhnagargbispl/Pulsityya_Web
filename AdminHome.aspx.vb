...
Imports System.IO
Partial Class App_UI_Application_Pages_AdminHome
...
" D.KitName,D.KitAmount,CASE WHEN a.ActiveStatus='Y' THEN ISNULL((Select SUM(RepurchIncome) From RepurchIncome With(NoLock) Where formno = A.formno),0) ELSE 0 END As Bv,a.panno,'Rs 0.00/-' As Balance,a.ActiveStatus,a.IsBlock,cm.CountryName,'true' as CountryVisible,cm.stdcode,'' as Aadharno,'' as acno,CASE WHEN a.fld5 = 'P' THEN 'Customer' ELSE 'Direct seller' END AS usertype From M_MemberMaster As A with(nolock) Inner Join M_StateDivMaster As B with(nolock) " & _
