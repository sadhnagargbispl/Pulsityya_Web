...
Partial Class App_UI_Application_Pages_AchieversMaster
    ...
    Public Sub BindData()
        Dim sql As String = "Select AcId,Cast(AcId as varchar) as VAcCode,MemberName,FormNo,IdNo,SessId,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status From Achievers  Order by SessId"
    ...
    Protected Sub DeleteGroup(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim AcId, scrname As String
        Dim GVRw As GridViewRow
        GVRw = CType(sender.Parent.Parent, GridViewRow)
        'GrpID = DirectCast(GVRw.FindControl("LblGrpID"), Label).Text
        AcId = DirectCast(GVRw.FindControl("LblGrpID"), Label).Text
        Dim Sql As String = "Update Achievers SET ActiveStatus='N' WHERE AcId='" & Val(AcId) & "' "
    ...
    '                    status = "Y"
    '                End If
    '                sql = "Select BankCode,Cast(BankCode as varchar) as VBankCode,BankName,IFSCode,AcNo,remarks,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status From " + objDAL.tblBankMaster + " Where " + objDAL.activeCondition + " AND ActiveStatus like '%" + status.ToString() + "%' Order by BankName"
    '            End If
    '        Else
    '            sql = "Select BankCode,Cast(BankCode as varchar) as VBankCode,BankName,IFSCode,AcNo,remarks,CASE WHEN ActiveStatus='Y' Then 'Active' ELSE 'DeActive' END AS Status From " + objDAL.tblBankMaster + " Where " + objDAL.activeCondition + " AND " + ddlSearchFields.SelectedValue.ToString() + " like '%" + txtSearch.Text + "%' Order by BankName"
    '        End If
    '        dtData = New DataTable
    '        dtData = objDAL.GetData(sql)
    '        GvData.DataSource = dtData
    ...
    Protected Sub btnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExport.Click
        Response.ClearContent()
        Response.Buffer = True
        Response.AddHeader("content-disposition", String.Format("attachment; filename={0}", "AchieverDetails.xls"))
        Response.ContentType = "application/ms-excel"
        Dim sw As New StringWriter()
        Dim htw As New HtmlTextWriter(sw)
        GvData.AllowPaging = False
