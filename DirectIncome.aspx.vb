...
Partial Class App_UI_Application_Pages_DirectIncome
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        Try
            If Session("AStatus") = "OK" Then
                Session("PageName") = "Report / Direct Income"
            Else
    ...
    Public Sub BindData(ByVal PageIndex As Integer)
        ...
        Try
            ...
            Else
                FromSessid = Session("CompDate")
                ToSessid = Format(Date.Now, "dd-MMM-yyyy")
            End If
            ...
            Else
                Idno = ""
            End If
            If Session("compid") = "1101" Or Session("compid") = "1104" Then
                If CheckBox1.Checked = True Then
                    Idno = txtMemId.Text
                Else
                    Idno = "0"
            ...
            If Session("compid") = "1103" Then
                ...
                ' -------- FROM DATE --------
                If txtStartDate.Text <> "" Then
                    Dim fromDate As DateTime = DateTime.ParseExact(txtStartDate.Text, "dd-MMM-yyyy", CultureInfo.InvariantCulture)
                    Dim fromDatePart As String = fromDate.ToString("yyyyMMdd")
                    ' FromDate ke liye hamesha session 01
                    FromSessid = fromDatePart & "01"
                Else
                    FromSessid = "0"
                End If
                ' -------- TO DATE --------
                If txtEndDate.Text <> "" Then
                    Dim toDate As DateTime = DateTime.ParseExact(txtEndDate.Text, "dd-MMM-yyyy", CultureInfo.InvariantCulture)
                    Dim toDatePart As String = toDate.ToString("yyyyMMdd")
                    ' Current time ke hisaab se session
                    Dim sessionNo As String
                    If DateTime.Now.TimeOfDay < New TimeSpan(12, 0, 0) Then
                        sessionNo = "01"
                    Else
                        sessionNo = "02"
                    End If

                    ToSessid = toDatePart & sessionNo
                Else
                    ToSessid = "0"
                End If
            ...
            Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), "Sp_ShowDirectIncomePayoutAdmin", prms)
    ...
    Private Sub ExportExcel()
        ...
        Using wb As New XLWorkbook()
            wb.Worksheets.Add(dt, "InvestmentIncome")
            Response.Clear()
            Response.Buffer = True
            Response.Charset = ""
            Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            Response.AddHeader("content-disposition", "attachment;filename=DirectIncome.xlsx")
