...
Partial Class App_UI_Application_Pages_Add100percent
    ...
    Protected Sub BtnFundTransfer_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnFundTransfer.Click
        Try
            ...
            Else
                ...
                query = "insert into M_100percent(Idno,ActiveStatus,RecTimeStamp,formno,sessid)values" & _
    ...
    Protected Sub FillDetail()
        ...
        Dim str As String = " select * from M_100percent where ActiveStatus='Y' "
        objDAL = New DAL(HttpContext.Current.Session("MlmDatabase" & Session("CompID")))
    ...
    Private Function ExistsIdNo() As Boolean
        Try
            Dim scrName As String
            Sql = "Select Count(*) as cnt From M_100percent WHERE IDNO='" & Trim(TxtIDNo.Text) & "'"
            ...
            If Dt.Rows(0)("cnt") >= 1 Then
                ...
                'LblMemName.ForeColor = Drawing.Color.Red
                scrName = "<SCRIPT language='javascript'>alert('Already Generate 100 Percent this Member ID!! ');" & "</SCRIPT>"
