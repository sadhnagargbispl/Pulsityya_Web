...
Imports System.IO
Imports System.Net
Partial Class MonthlyWalletauth
    ...
    Public Sub BindSession()
        Dim sql As String = "Select * From(Select 0 As SessID,'-- ALL --' As SessnName Union ALL select SessID,'Withdrawal Month '+Cast(SessID As Varchar)+' Date : '+ Replace(Convert(Varchar, frmDate,106),' ','-') " & _
        " +' to '+ Replace(Convert(Varchar, ToDate,106),' ','-') As SessnName from M_MonthSessnmaster Where ToDate Is Not Null) As Temp order by SessID"
    ...
    Protected Sub btnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExport.Click
        Try
            ...
            ExportToExcel("MonthlyWalletAuthentication.xls", dg)
    ...
    Protected Sub btnConfirm_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnConfirm.Click
        ...
        'Dim Sql As String = "Update PlanBwithdrawls Set Status='A',IssueDate=GETDATE(),Remark='" & TxtRemark.Text & "',UserId='" & Val(Session("UserID")) & "',
        ...
        " insert into UserHistory(UserId,UserName,PageName,Activity,ModifiedFlds,RecTimeStamp,MemberId)Values" & _
         "('" & Val(Session("UserID")) & "','" & Session("UserName") & "','Monthly Bank WithDrawals','Monthly Bank Withdrawls Approve','" & Remark & "',Getdate(),'" & LblForm1.Text & "')"
    ...
    Protected Sub btnApprove_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnApprove.Click
        ...
        For Each Gvr As GridViewRow In GvData.Rows
            ...
            If Chk.Checked Then
                ...
                Dim Remark As String = ""
                Remark = "Withdrawal Approved Of Idno " & TxtIDNo.Text & " For MonthNo:" & LnblWeek.Text & " By " & Session("UserName") & ""
                Dim Sql As String = "Update MonthWithdrawls Set Status='A',IssueDate=GETDATE(),Remark='" & txtRemark.Text & "',UserId='" & Val(Session("UserID")) & "',UserName='" & Session("UserName") & "' Where Sno=" & LblId.Text & ";" & _
               ...
               " insert into UserHistory(UserId,UserName,PageName,Activity,ModifiedFlds,RecTimeStamp,MemberId)Values" & _
         "('" & Val(Session("UserID")) & "','" & Session("UserName") & "','Monthly Bank WithDrawals','Monthly Bank Withdrawls Approve','" & Remark & "',Getdate(),'" & FormNo & "')"
    ...
    Protected Sub BtnApproveAll_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnApproveAll.Click
        ...
        For Each Gvr As GridViewRow In GvData.Rows
            ...
            If Chk.Checked Then
               ...
               " insert into UserHistory(UserId,UserName,PageName,Activity,ModifiedFlds,RecTimeStamp,MemberId)Values" & _
         "('" & Val(Session("UserID")) & "','" & Session("UserName") & "','Month Withdrawls','Month Withdrawls Approve','" & Remark & "',Getdate(),'" & FormNo & "')"
    ...
    Protected Sub BtnExportToCsv_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnExportToCsv.Click
        ...
        '     " ,Case when a.Status='P' Then '' else A.Remark end As Remark,UserName as ProcessedBy,Replace(Convert(varchar,IssueDate,106),' ','-') + ' '+ CONVERT(varchar(15),CAST(IssueDate AS TIME),100) as ProcessedDate,A.WSessID as WeekNo" & _
        '    " ,Replace(Convert(Varchar,d.FrmDate,106),' ','-') as FromDate,Replace(Convert(varchar,d.ToDate,106),' ','-') as ToDate, " & _
        '    " ''''+A.AcNo as AcNo, A.IFSCode, B.BankName, A.BranchName, A.PanNo From PlanBwithdrawls As A Left Join M_BankMaster As B On  A.BankID=B.BankCode And " & _
        ...
        " from MonthWithdrawls as a,M_memberMaster as b   where a.Formno = b.Formno " & Condition & " Order by Sno "
    ...
    Protected Sub btnRejectAll_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnRejectAll.Click
        ...
        For Each Gvr As GridViewRow In GvData.Rows
            ...
            If Chk.Checked Then
                Dim Id As String = DirectCast(Gvr.FindControl("LblID"), Label).Text
                Dim Id1 As String = DirectCast(Gvr.FindControl("Label1"), Label).Text
                Dim FormNo As String = DirectCast(Gvr.FindControl("LblFormNo"), Label).Text
                Dim DateOn As String = DirectCast(Gvr.FindControl("LblDate"), Label).Text
                ...
                Remark = "Withdrawal Rejected Of Idno " & IdNo & " for MonthNo:" & WeekNo & " By " & Session("UserName") & ""

        
                If Session("compid") = "1038" Then
                    sql = sql & "Update MonthWithdrawls Set Status='R',IssueDate=GETDATE(),Remark='" & TxtRemark & "',UserId='" & Val(Session("UserID")) & "'," & _
                            "UserName='" & Session("UserName") & "' Where ReqID=" & Id1 & ";"
                    sql = sql & " INSERT INTO TrnVoucher(VoucherNo,VoucherDate,DrTo,CrTo,Amount,Narration,RefNo,AcType,VTYpe,SessID,WSessID) "
                    sql = sql & "  SELECT ISNULL(Max(VoucherNo)+1,1001),GETDATE(),0, '" & FormNo & "'," & Val(LblAmount) & ","
                    sql = sql & "  'Withdrawal Rejected  Req. No. " & Id1 & "','Req/" & Id1 & "','J','C',convert(varchar,getdate(),112),"
                    ...
                    sql = sql & " insert into UserHistory(UserId,UserName,PageName,Activity,ModifiedFlds,RecTimeStamp,MemberId)Values" & _
                                "('" & Val(Session("UserID")) & "','" & Session("UserName") & "','Fund WithDrawals','Fund Withdrawls Reject'," & _
                                "'" & Remark & "',Getdate(),'" & FormNo & "')"
                Else
                    sql = sql & "Update MonthWithdrawls Set Status='R',IssueDate=GETDATE(),Remark='" & TxtRemark & "',UserId='" & Val(Session("UserID")) & "',UserName='" & Session("UserName") & "' Where Sno=" & Id & ";" & _
" Delete From TrnVoucher Where DrTo='" & FormNo & "' And vtype='W' And VoucherDate='" & DateOn & "' And Narration Like 'Fund Debited Againest Bank Withdrawal on " & DateOn & "';"
                    sql = sql & " insert into UserHistory(UserId,UserName,PageName,Activity,ModifiedFlds,RecTimeStamp,MemberId)Values" & _
                            "('" & Val(Session("UserID")) & "','" & Session("UserName") & "','Month WithDrawals','Month Withdrawls Reject','" & Remark & "',Getdate(),'" & FormNo & "')"
    ...
    Protected Sub btnReject_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReject.Click
        ...
        For Each Gvr As GridViewRow In GvData.Rows
            ...
            If Chk.Checked Then
                ...
                Dim WeekNo As String = DirectCast(Gvr.FindControl("LblWeek"), Label).Text
                Remark = "Withdrawal Rejected Of Idno " & IdNo & " for MonthNo:" & WeekNo & " By " & Session("UserName") & ""
                sql = sql & "Update MonthWithdrawls Set Status='R',IssueDate=GETDATE(),Remark='" & TxtARemark.Text & "',UserId='" & Val(Session("UserID")) & "',UserName='" & Session("UserName") & "' Where Sno=" & Id & ";" & _
  " Delete From TrnVoucher Where DrTo='" & FormNo & "' And vtype='W' And VoucherDate='" & DateOn & "' And Narration Like 'Fund Debited Againest Bank Withdrawal on " & DateOn & " with req. no." & Id & "';"
    ...
    Protected Sub btnRejectSingle_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnRejectSingle.Click
        ...
        Dim Remark As String = ""
        Remark = "Withdrawal Rejected Of Idno " & LblRejIdNo.Text & " for MonthNo:" & LblRejWeek.Text & " By " & Session("UserName") & ""
        Dim Sql As String = "Update MonthWithdrawls Set Status='R',IssueDate=GETDATE(),Remark='" & TxtARemark.Text & "',UserId='" & Val(Session("UserID")) & "',UserName='" & Session("UserName") & "' Where Sno=" & LblRejReqNo.Text & ";Delete From TrnVoucher Where DrTo='" & lblRejFormno.Text & "' And vtype='W' And VoucherDate='" & LblRejDateOn.Text & "' And Narration Like 'Fund Debited Againest Bank Withdrawal on " & LblRejDateOn.Text & " with req. no." & LblRejReqNo.Text & "';"

        Sql = Sql & " insert into UserHistory(UserId,UserName,PageName,Activity,ModifiedFlds,RecTimeStamp,MemberId)Values" & _
         "('" & Val(Session("UserID")) & "','" & Session("UserName") & "','Month WithDrawals','Month Withdrawls Reject','" & Remark & "',Getdate(),'" & lblRejFormno.Text & "')"
        If objDAL.SaveData(Sql) <> 0 Then
