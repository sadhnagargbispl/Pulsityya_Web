...
Partial Class ApprovePaymentReqsCompwise
...
"ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]) ON [PRIMARY] ALTER TABLE [dbo].[Trnfundtransferbyadmin] ADD  DEFAULT (getdate()) FOR [Rectimestamp] ')"
    ...
    Public Sub BindData(Optional ByVal Condition As String = "")
        ...
        sql &= "Case when ScannedFile='' then '' else MlmUrl + 'images/UploadImage/" & Session("CompId") & "/'+ ScannedFile end as ScannedFile "
        sql &= " ,Case when ScannedFile='' then 'False' else 'True' end as ScannedFileStatus,ApproveRemark,'' as txnhash "
        sql &= "from WalletReq as a with(nolock),M_BankMaster as b with(nolock), M_MemberMaster as c with(nolock),m_companymaster as p with(nolock) "
    ...
    Private Sub AprvAction(ByVal AprvType As String, ByVal ApprvStatus As String)
        ...
        Dim StrSql_ As String = "Insert into Trnrejectbyadmin (Transid,Rectimestamp) values(" & HdnCheckTrnns.Value & ",getdate())"
        updateeffect = objDAL.SaveData(StrSql_)
        If updateeffect > 0 Then
            ...
            If a <> 0 And Cnt > 0 Then
                ...
                TxtARemark.Text = ""
                ScriptManager.RegisterStartupScript(Me, Me.[GetType](), "Key", "alert('" & lblMsg.Text & "');location.replace('ApprovePaymentReqsCompwise.aspx');", True)
                BindData()
            Else
            ...
            End If
        Else
            ScriptManager.RegisterStartupScript(Me, Me.[GetType](), "Key", "alert('Try After Some Time.');location.replace('ApprovePaymentReqsCompwise.aspx');", True)
        End If
    End Sub
