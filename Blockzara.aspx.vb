Imports System.Data
Partial Class App_UI_Application_Pages_Blockzara
...
#End Region
    ...
    Private Sub ShowDetail()
        ...
        If txtMemberId.Text <> "" Then
            ...
            If rdblistChoice.SelectedValue = "single" Then
                qry = "Select IDNO as [Member ID],MemName as [Member Name],Replace(Convert(varchar,DOJ,106),' ','-') as [Join Date],KitName as [Kit Name],Replace(Convert(varchar,a.UpgradeDate,106),' ','-') as [Activ.Date],a.ReferalIDNo as [Sponsor],a.ReferralName as [Sponsor Name],BlockStatus as [Block Status],a.BlockRemark from V#MemberDetail as a Where IDNO like '" & idNo & "'"

                'qry = qry & " insert into UserHistory(UserId,UserName,PageName,Activity,ModifiedFlds,RecTimeStamp)Values" & _
                '     "('" & Val(Session("UserID")) & "','" & Session("UserName") & "','Block ','Block','" & Remark & "',Getdate())"

            Else
                qry = "Select a.IDNO as [Member ID],a.MemName as [Member Name],Replace(Convert(varchar,a.DOJ,106),' ','-') as [Join Date],a.KitName as [Kit Name],Replace(Convert(varchar,a.UpgradeDate,106),' ','-') as [Activ.Date],a.ReferalIDNo as [Sponsor],a.ReferralName as [Sponsor Name],BlockStatus as [Block Status],a.BlockRemark from V#MemberDetail as a,M_MemTreeRelation as b Where b.FormNoDwn=a.FormNo AND b.FormNo='" & Trim(TxtFormNo.Text) & "'"
            End If
   ...
   "('" & Val(Session("UserID")) & "','" & Session("UserName") & "','Block ','Block Tree','" & Remark & "',Getdate(),'" & Trim(TxtFormNo.Text) & "')"
            Sql = Sql & "; Insert Into TempMemberMaster Select *,'Block Tree',GetDate(),'B' From M_MemberMaster Where FormNo='" & Trim(TxtFormNo.Text) & "'"
            scrname = "Tree"
        End If
