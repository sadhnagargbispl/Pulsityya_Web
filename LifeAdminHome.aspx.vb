...
Partial Class LifeAdminHome
    ...
    Public Sub BindData(Optional ByVal Condition As String = "")
        ...
        url = IIf(tempProtocol.StartsWith("http://"), "https://", "https://") & HttpContext.Current.Request.Url.Host.ToUpper().Replace("HTTP://", "").Replace("HTTPS://", "").Replace("WWW.", "").Replace("BASICMLM.", "").Replace("ADMIN.", "CPANEL.")
...
" D.KitName,isnull(D.KitAmount,0)As KitAmount,isnull(vo.Pv,D.Bv)As BV,'Rs 0.00/-' As Balance,a.ActiveStatus,a.IsBlock,'' as CountryName,'false' as CountryVisible,'' as StdCode From M_MemberMaster As A Inner Join M_StateDivMaster As B on A.StateCode=B.StateCode and  b.RowStatus='Y' " & _
" Left Join M_MemberMaster As C On A.RefFormNo=C.FormNo Left Join M_MemberMaster As e On A.UpLnFormNo=e.Formno " & _
" Left Join V#OrderPV as vo ON a.Formno=vo.formno and a.Kitid=vo.kitid Inner join  " & _
" M_kitMaster As D On A.KitID=D.KitID " & Condition & " Order by A.Doj Desc"
        Else
            sql = "Select top 500  A.IDNo,  A.Prefix +' '+ A.MemFirstName + ' '+ A.MemLastName As Name1 ,A.MemFirstName As MemName,Case when A.LegNo=1 then 'Left' else 'Right' end as Leg,'' as LgnID,'" & url & "' as Site," & _
   " IsNull(REPLACE(CONVERT(VARCHAR(11), a.Doj , 106), ' ', '-')+' '+STUFF(RIGHT( CONVERT(VARCHAR,a.Doj ,100 ) ,7), 6, 0, ' '),'') As Doj,A.City,B.StateName,A.Email,A.Mobl As MobileNo," & _
   "A.Passw,C.IDNo As SponsorID,(C.MemFirstName +' '+C.MemLastName) as SponsorName, e.IdNo as UplinerIdNo, (e.MemFirstName+ ''+ e.MemLastName) as UplinerName, " & _
...
" ELSE  A.MemFirstName  END AS Qstr, Case When A.ActiveStatus='Y' and a.IsBlock='N' then 'Active' when a.IsBlock='Y' then 'Blocked' Else 'Pending' End As Status," & _
" CASE WHEN a.ActiveStatus='Y' THEN ISNULL( REPLACE(CONVERT(VARCHAR(11), a.UpgradeDate , 106), ' ', '-') +' '+STUFF(RIGHT( CONVERT(VARCHAR,a.UpgradeDate ,100 ) ,7), 6, 0, ' '),'') ELSE '' END as UpgrdDate," & _
" D.KitName,isnull(Vo.OrderAmt,D.KitAmount)As KitAmount,isnull(vo.Pv,D.Bv)As BV,'Rs 0.00/-' As Balance,a.ActiveStatus,a.IsBlock,'' as CountryName,'false' as CountryVisible,'' as StdCode From M_MemberMaster As A Inner Join M_StateDivMaster As B on A.StateCode=B.StateCode and  b.RowStatus='Y' " & _
" Left Join M_MemberMaster As C On A.RefFormNo=C.FormNo Left Join M_MemberMaster As e On A.UpLnFormNo=e.Formno " & _
" Left Join V#OrderPV as vo ON a.Formno=vo.formno and a.Kitid=vo.kitid Inner join  " & _
" M_kitMaster As D On A.KitID=D.KitID " & Condition & " Order by A.Doj Desc"
        End If
        ...
        Try
            ...
            If Session("CompID") = 1075 Then
                sql = "Select Top 500  A.IDNo,A.MemFirstName As MemName,C.IDNo As SponsorId,(C.MemFirstName +' '+C.MemLastName) as SponsorName," & _
                                              " A.Email,A.Mobl As MobileNo,D.KitName as packageName,IsNull(REPLACE(CONVERT(VARCHAR, a.Doj , 106), ' ', '-'),' ') as DojDate," & _
                                   ...
                                   " Inner Join M_StateDivMaster As B with(nolock) on A.StateCode=B.StateCode and b.RowStatus='Y'  " & _
                                   " Left Join M_MemberMaster As C with(nolock) On A.RefFormNo=C.FormNo Left Join M_MemberMaster As e " & _
                                   " On A.UpLnFormNo=e.Formno Left Join V#OrderPv as Vo on vo.formno=a.formno and a.Kitid=vo.kitid Inner join M_kitMaster As D with(nolock) On A.KitID=D.KitID " & Condition & " Order by A.Doj Desc"

            Else
                sql = "Select Top 500  A.IDNo,A.MemFirstName As MemName,C.IDNo As SponsorId,(C.MemFirstName +' '+C.MemLastName) as SponsorName," & _
                                              " A.Email,A.Mobl As MobileNo,D.KitName as packageName,isnull(vo.OrderAmt,D.KitAmount) as PackageMRP,isnull(vo.pv,D.Bv) as [PV/EP],IsNull(REPLACE(CONVERT(VARCHAR, a.Doj , 106), ' ', '-'),' ') as DojDate," & _
                                   " STUFF(RIGHT( CONVERT(VARCHAR,a.Doj ,100 ) ,7), 6, 0, ' ') As DojTime,A.City,B.StateName,A.Passw, " & _
                                   " CASE WHEN a.ActiveStatus='Y' THEN ISNULL( REPLACE(CONVERT(VARCHAR, a.UpgradeDate , 106), ' ', '-'),'') ELSE '' END as UpgrdDate," & _
                                   " CASE WHEN a.ActiveStatus='Y' THEN STUFF(RIGHT( CONVERT(VARCHAR,a.UpgradeDate ,100 ) ,7), 6, 0, ' ') ELSE '' END as Upgrdtime,Case When A.ActiveStatus='Y' " & _
                                   " and a.IsBlock='N' then 'Active' when a.IsBlock='Y' then 'Blocked' Else 'Pending' End As Status,'Rs 0.00/-' As Balance,a.ActiveStatus From M_MemberMaster As A with(nolock) " & _
