...
Partial Class App_UI_Application_Pages_FranchiseDetailsReport
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        If Session("AStatus") = "OK" Then
            Session("PageName") = "Franchise / Franchise Details Report"
        Else
    ...
    '            strsql = "sp_GetWallettype_Rep"
    '        ElseIf (Session("CompId") = "1039") Then
    '            strsql = "sp_WalletTransactionType"

    '        Else
    '            strsql = "sp_GetWallettype"
    '        End If

    '        Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), strsql)
    ...
    '        'RbtWalletType.Items.Insert(0, "All")
    ...
    Private Sub FillState()
        Try
            ...
            Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), strsql)
            RbtWalletType.DataSource = Ds.Tables(0)
            RbtWalletType.DataValueField = "StateCode"
            RbtWalletType.DataTextField = "Statename"
            RbtWalletType.DataBind()


            'objDAL = New DAL(HttpContext.Current.Session("MlmDatabase" & Session("CompID")))
            'strsql = "Select Statecode,Statename from M_StateDivmaster where ActiveStatus='Y' and RowStatus='Y'"
            'dtData = New DataTable
            'dtData = objDAL.GetData(strsql)
            'If dtData.Rows.Count > 0 Then
            '    RbtWalletType.SelectedValue = dtData.Rows(0)("Statecode")
            '    RbtWalletType.SelectedItem.Text = dtData.Rows(0)("Statename")
    ...
    Private Function GetFormNo() As String
        Try
            ...
            idNo = idNo.Trim
            Dim qry As String = "Select FormNo,idno from " & objDAL.tblMemberMaster & " where IdNo='" & idNo & "'"
            Dim dt As New DataTable
            dt = objDAL.GetData(qry)
            If (dt.Rows.Count > 0) Then
                formno = dt.Rows(0)("idno")
            Else
    ...
    Protected Sub GvData_PageIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles GvData.PageIndexChanging
        ...
        GvData.DataSource = Session("FranchiseData")
        GvData.DataBind()
    End Sub
    Private Sub ExportExcel()
        Dim dt As DataTable = Session("FranchiseData1")
        Using wb As New XLWorkbook()
            wb.Worksheets.Add(dt, "FranchiseDetails")
            Response.Clear()
            Response.Buffer = True
            Response.Charset = ""
            Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            Response.AddHeader("content-disposition", "attachment;filename=FranchiseDetails.xlsx")
    ...
    Protected Sub btnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExport.Click
        ...
        Try
            ...
            'Dim Statename As String = RbtWalletType.SelectedValue
            Dim Statename As String = ""
            If RbtWalletType.SelectedValue <> 0 Then
                Statename = RbtWalletType.SelectedItem.Text
            Else
                Statename = ""
            End If
            ...
            If (Session("CompID") = "1039") Then
                'str = "sp_GetWalletTransactionDetailNew"
            Else
                str = "sp_franchisedetails"
            End If
            Dim prms As SqlParameter() = New SqlParameter(7) {}
            prms(0) = New SqlParameter("@IDNo", Convert.ToString(Idno).ToLower())
            prms(1) = New SqlParameter("@Statename", Statename)
            ...
            'Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), str, prms)
            Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), str, prms)
            'Session("AccountData1") = Ds.Tables(0)
            Session("FranchiseData1") = Ds.Tables(0)
    ...
    Public Sub BindData(ByVal PageIndex As Integer)
        ...
        Try
            ...
            If (Session("CompID") = "1039") Then
                str = "sp_GetWalletTransactionDetailNew"
            Else
                'str = "sp_GetWalletTransactionDetail"
                str = "sp_franchisedetails"
            End If
            ...
            GvData.DataBind()
            Dim prms As SqlParameter() = New SqlParameter(7) {}
            'Dim prms As SqlParameter() = New SqlParameter(4) {}
            prms(0) = New SqlParameter("@IDNo", Convert.ToString(Idno).ToLower())
            prms(1) = New SqlParameter("@Statename", Statename)
            prms(2) = New SqlParameter("@StartDate", Convert.ToDateTime(startDate))
            prms(3) = New SqlParameter("@EndDate", Convert.ToDateTime(endDate))
            ...
            prms(7) = New SqlParameter("@RecordCount", ParameterDirection.Output)
            '' sp_GetWalletTransactionDetail '0','S','10/19/2020','10/20/2020',1,10,'N',10 --- For Test in Sql Server 
            'Commit 29 Jan 2022
            'Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), "sp_franchisedetails", prms)
            Ds = SqlHelper.ExecuteDataset(HttpContext.Current.Session("MlmDatabase" & Session("CompID")), str, prms)
            GvData.DataSource = Ds.Tables(0)
            GvData.DataBind()
            'below commit 22 April 2022
            Dim recordCount As Integer = Ds.Tables(1).Rows(0)("RecordCount")
            'Session("AccountData") = Ds.Tables(0)
            Session("FranchiseData") = Ds.Tables(0)
            ViewState("IdNo") = "IdNo"
    ...
    Private Sub RebindData(ByVal sColimnName As String, ByVal sSortOrder As String)
        Try
            ...
            Dim dt As DataTable = CType(Session("FranchiseData"), DataTable)
    ...
    '        CType(e.Row.FindControl("Image"), Image).ImageUrl = imageUrl
    '    End If
    'End Sub
End Class
