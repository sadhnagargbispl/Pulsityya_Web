...
Partial Class ApporvePaymentReqSZara
    ...
    Public Sub Fillwallet()
        Try
            Dim Sql As String = " select * from (select 0 as Acid,'--Choose WalletName--' as WalletName, '' as Actype  Union all " & _
       " Select Acid,WalletName,Actype From Vouchertype  Where  Activestatus='Y' AND acid in (2,6))as a where  1=1 Order By acid"

            Dim Dt As New DataTable
            Dt = objDAL.GetData(Sql)
            DDlKit.DataSource = Dt
            DDlKit.DataTextField = "WalletName"
            DDlKit.DataValueField = "Actype"
            DDlKit.DataBind()
            Session("MKit") = Dt
    ...
    Public Sub BindData(Optional ByVal Condition As String = "")
        ...
        If DDlKit.SelectedValue <> "" Then
            Condition = Condition & " And d.Actype='" & DDlKit.SelectedValue & "'"
        ...
        " end as status,Case when IsApprove='N' then 'True' else 'False' end as EnableStatus,Amount,a.Remarks, CASE WHEN ScannedFile='' THEN '' WHEN ScannedFile like 'http%' THEN ScannedFile else 'https://cpanel.trueway.online/images/UploadImage/'+ScannedFile END AS ScannedFile " & _
        " ,Case when ScannedFile='' then 'False' else 'True' end as ScannedFileStatus,ApproveRemark,d.walletname,d.Actype from WalletReq " & _
        " as a  with(nolock) inner join M_BankMaster as b  with(nolock) on a.BankId=b.BankCode left join VoucherType as d on a.Actype=d.Actype," & _
        " M_MemberMaster as c  with(nolock) where   a.Formno=c.Formno and b.RowStatus='Y' " & Condition & " order by ReqNo  "
            'where a.Actype=d.Actype AND a.Formno=c.Formno and  a.BankId=b.BankCode and b.RowStatus='Y' " & Condition & "order by ReqNo "
        ElseIf Session("CompId") = "1084" Then
            sql = " select a.Formno,c.Idno,(c.MemFirstName+ ' '+c.MemlastName) as Membername,Email as Email ,Mobl as Mobileno,ReqNo,A.actype," & _
       " Replace(Convert(Varchar,a.RectimeStamp,106),' ','-')+ ' '+  " & _
       " CONVERT(varchar(15), CAST(a.RectimeStamp AS TIME),100) as ReqDate,a.PayMode,a.Chqno,Replace(Convert(Varchar,a.ChqDate,106),' ','-') as ChequeDate," & _
       " b.BankName,a.Branchname,Case when IsApprove='N' then 'Pending' when IsApprove='Y' then 'Approve' else 'Rejected'" & _
       " end as status,Case when IsApprove='N' then 'True' else 'False' end as EnableStatus,Amount,a.Remarks, CASE WHEN ScannedFile='' THEN '' WHEN ScannedFile like 'http%' THEN ScannedFile else 'https://cpanel.ecoflowsglobal.com/images/UploadImage/'+ScannedFile END AS ScannedFile " & _
       " ,Case when ScannedFile='' then 'False' else 'True' end as ScannedFileStatus,ApproveRemark,d.walletname,d.Actype from WalletReq " & _
       " as a  with(nolock) inner join M_BankMaster as b  with(nolock) on a.BankId=b.BankCode left join VoucherType as d on a.Actype=d.Actype," & _
       " M_MemberMaster as c  with(nolock) where   a.Formno=c.Formno and b.RowStatus='Y' " & Condition & " order by ReqNo  "
            ...
            For Each Gvr As GridViewRow In GvData.Rows
                ...
                If Chk.Checked = True And Chk.Enabled = True Then
                    ...
                    If (Val(Dt1.Rows(0)("Cnt")) = 0) Then
                        If AprvType = "Y" Then
                            If Session("CompId") = 1066 Or Session("CompId") = 1068 Then
    ...
    Public Function SendToMemberMail() As Boolean
        Try
            ...
            StrMsg = "<table style=""margin:0; padding:10px; font-size:12px; font-family:Verdana, Arial, Helvetica, sans-serif; line-height:23px; text-align:justify;width:100%""> " & _
                     "<tr>" & _
                     "<td>" & _
"Your Wallet Request number <span style=""font-weight: bold;"">" & Session("Reqno") & "</span> from <span style=""font-weight: bold;"">" & Session("Idno") & "</span> has been approved on  " & Session("GETDATE") & ". Please check your wallet.<br />" & _
    ...
    Public Function SendToMemberMailRejected() As Boolean
        Try
            ...
            StrMsg = "<table style=""margin:0; padding:10px; font-size:12px; font-family:Verdana, Arial, Helvetica, sans-serif; line-height:23px; text-align:justify;width:100%""> " & _
                     "<tr>" & _
                     "<td>" & _
"Your Wallet Request number <span style=""font-weight: bold;"">" & Session("Reqno") & "</span> from <span style=""font-weight: bold;"">" & Session("Idno") & "</span> has been Rejected on  " & Session("GETDATE") & " because of " & Session("remark") & " <br />" & _
"<span style=""color: #0099FF; font-weight: bold;"">Regards,</span><br />" & _
                     "<a href=""" & Session("CompWeb") & """ target=""_blank"" style=""color:#0000FF; text-decoration:underline;"">" & Session("CompName") & "</a><br />" & _
                     "<br />" & _
                     "<br />" & _
    ...
    Protected Sub btnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExport.Click
        ...
        Try
            ...
            Dim sql As String = " select a.Formno,c.Idno,(c.MemFirstName+ ' '+c.MemlastName) as Membername,Mobl as Mobileno,Email,ReqNo," & _
            " Replace(Convert(Varchar,a.RectimeStamp,106),' ','-')+ ' '+  " & _
                           " CONVERT(varchar(15), CAST(a.RectimeStamp AS TIME),100) as ReqDate,a.PayMode,d.walletname,''''+ Convert(Varchar,a.Chqno) As Chqno,Replace(Convert(Varchar,a.ChqDate,106),' ','-') as ChequeDate," & _
                " b.BankName,a.Branchname,Case when IsApprove='N' then 'Pending' when IsApprove='Y' then 'Approve' else 'Rejected'" & _
              ...
              " as a  with(nolock) inner join M_BankMaster as b  with(nolock) on a.BankId=b.BankCode left join VoucherType as d on a.Actype=d.Actype, M_MemberMaster as c  with(nolock) where a.Formno=c.Formno and b.RowStatus='Y' " & Condition & " order by ReqNo "
            dtData = New DataTable
            dtData = objDAL.GetData(sql)
            'GvData.DataSource = dtData
            'GvData.DataBind()
